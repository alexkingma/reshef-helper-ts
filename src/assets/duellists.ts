const routeDuellistNames: Duellist["name"][] = [
  "Player (Initial Deck)",
  "Player (Initial Trunk)",
  "Conductor (Turnstyles)",
  "Conductor (Carriage)",
  "PaniK 1",
  "Millennium Guardian 1",
  "Duel Computer (Novice)",
  "Puppeteer of Doom",
  "Jean-Claude Magnum 1",
  "Millennium Guardian 2",
  "Bonz 1",
  "Millennium Guardian 3",
  "Para 1",
  "Para 2",
  "Dox 1",
  "Millennium Guardian 4",
  "Jinzo",
  "Rex 1",
  "Millennium Guardian 5",
  "Mimic of Doom 1",
  "Rare Hunter 1",
  "Ghoul at Bonz",
  "Ghoul right of Rex",
  "Ghoul left of Rex",
  "Strings",
  "Lumis",
  "Umbra",
  "Arkana",
  "Rare Hunter 2",
  "Mini Ghoul 1",
  "Mini Ghoul 2",
  "Mini Ghoul 3",
  "Mini Ghoul 4",
  "Mini Ghoul 5",
  "Bandit Keith",
  "Marik Ishtar",
  "Paradox Guardian 1",
  "Paradox Guardian 2",
  "Paradox Guardian 3",
  "Paradox Guardian 4",
  "Paradox Guardian 5",
  "Paradox",
  "Chevaliers (Cable Car)",
  "PaniK 2",
  "Mimic of Doom 2",
  "Chevalier 3 (West)",
  "Chevalier 3 (East)",
  "Para 3",
  "Dox 2",
  "Bonz 3",
  "Rex 3",
  "Weevil 3",
  "Mako 3",
  "Mai 3",
  "Espa 2",
  "Bonz (Imposter)",
  "Rex (Imposter)",
  "Weevil (Imposter)",
  "Mako (Imposter)",
  "Mai (Imposter)",
  "Espa (Imposter)",
  "Dark Mokuba",
  "Seto Kaiba",
  "Sol Chevalsky",
  "Reshef",
];

const duellists: Duellist[] = [
  {
    name: "Player (Initial Deck)",
    deck: {
      "Armed Ninja": 1,
      "Dancing Elf": 1,
      "Monster Eye": 1,
      "Ancient Jar": 1,
      "Magician of Faith": 1,
      "Mystic Lamp": 1,
      "Hane-Hane": 1,
      "Jinzo #7": 1,
      "Mechanical Spider": 1,
      Tomozaurus: 1,
      "Flame Dancer": 1,
      "Star Boy": 1,
      "Candle of Fate": 1,
      "Curtain of the Dark Ones": 1,
      "Droll Bird": 1,
      "Hinotama Soul": 1,
      "LaLa Li-oon": 1,
      "Weather Control": 1,
      "Boo Koo": 1,
      Kaminarikozou: 1,
      "Petit Dragon": 1,
      "Phantom Dewan": 1,
      "The Bewitching Phantom Thief": 1,
      "Wicked Mirror": 1,
      "Winged Cleaver": 1,
      "Cyber Commander": 1,
      "Lightning Conger": 1,
      "Morphing Jar #2": 1,
      "The Furious Sea King": 1,
      Genin: 1,
      "Doma The Angel of Silence": 1,
      Tatsunootoshigo: 1,
      Peacock: 1,
      Leogun: 1,
      "Beast of Talwar": 1,
      "Beckon to Darkness": 1,
      "Dark-Piercing Light": 1,
      "Final Destiny": 1,
      Sparks: 1,
      Eatgaboon: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Player (Initial Trunk)",
    deck: {
      "Thousand-Eyes Idol": 2,
      "The Unhappy Maiden": 2,
      "Zone Eater": 2,
      "Skull Servant": 2,
      "Steel Scorpion": 2,
      "Dancing Elf": 2,
      Ooguchi: 2,
      "Armed Ninja": 2,
      "Sinister Serpent": 2,
      "Milus Radiant": 2,
      Bat: 2,
      Ameba: 2,
      Griggle: 2,
      "Queen's Double": 2,
      "Dark Plant": 2,
      "Ancient Jar": 2,
      "Key Mace": 2,
      "Change Slime": 2,
      "Magician of Faith": 2,
      "Bone Mouse": 2,
      "Shadow Specter": 2,
      "Pot the Trick": 2,
      Tomozaurus: 2,
      Haniwa: 2,
      "Mechanical Spider": 2,
      "Jinzo #7": 2,
      "White Dolphin": 2,
      "Turu-Purun": 2,
      "Rainbow Flower": 2,
      "Hane-Hane": 2,
      "Bite Shoes": 2,
      Korogashi: 2,
      "Flame Dancer": 2,
      "Star Boy": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Tristan Taylor",
    deck: {
      Griggle: 3,
      "The Unhappy Maiden": 3,
      "Sinister Serpent": 3,
      "Milus Radiant": 3,
      "Armed Ninja": 3,
      "Queen's Double": 3,
      "Bone Mouse": 1,
      "Change Slime": 1,
      "Dancing Elf": 3,
      Ooguchi: 3,
      "Key Mace": 1,
      "Steel Scorpion": 3,
      Ameba: 3,
      Bat: 3,
      "Zone Eater": 3,
      "Ancient Jar": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Game Shop",
    payout: 0,
    ante: [],
  },
  {
    name: "Yugi Muto",
    deck: {
      "Gaia the Dragon Champion": 1,
      "Shadow Specter": 3,
      "Mushroom Man": 3,
      "Hane-Hane": 3,
      "Ancient Jar": 3,
      "Curse of Dragon": 1,
      Kuriboh: 3,
      "Gaia The Fierce Knight": 1,
      "Exodia the Forbidden One": 1,
      "Left Arm of the Forbidden One": 1,
      "Right Arm of the Forbidden One": 1,
      "Left Leg of the Forbidden One": 1,
      "Right Leg of the Forbidden One": 1,
      "Skull Servant": 3,
      "Summoned Skull": 1,
      "Dark Magician": 1,
      "Man-Eater Bug": 3,
      Sangan: 1,
      "Monster Reborn": 1,
      "Spellbinding Circle": 2,
      "Horn of the Unicorn": 1,
      "Mystical Moon": 1,
      "Book of Secret Arts": 1,
      Eatgaboon: 1,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Game Shop",
    payout: 0,
    ante: [
      "Right Leg of the Forbidden One",
      "Left Leg of the Forbidden One",
      "Right Arm of the Forbidden One",
      "Left Arm of the Forbidden One",
      "Exodia the Forbidden One",
      "Kuriboh",
      "Spellbinding Circle",
    ],
  },
  {
    name: "Joey Wheeler",
    deck: {
      "Battle Warrior": 3,
      "Kagemusha of the Blue Flame": 3,
      "Baby Dragon": 1,
      "Jinzo #7": 3,
      "Time Wizard": 3,
      "The Unhappy Maiden": 3,
      "Mountain Warrior": 3,
      "Flame Swordsman": 1,
      "Magician of Faith": 3,
      "Thousand Dragon": 1,
      "Swordsman of Landstar": 3,
      "Armed Ninja": 3,
      "Dancing Elf": 3,
      "Red-Eyes B. Dragon": 1,
      "Queen's Double": 3,
      "Kunai with Chain": 1,
      Salamandra: 1,
      Eatgaboon: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Game Shop",
    payout: 0,
    ante: [
      "Baby Dragon",
      "Flame Swordsman",
      "Time Wizard",
      "Mountain Warrior",
      "Thousand Dragon",
      "Battle Warrior",
      "Kunai with Chain",
      "Salamandra",
      "Swordsman of Landstar",
    ],
  },
  {
    name: "Rebecca",
    deck: {
      "Blast Juggler": 3,
      "Witch of the Black Forest": 1,
      "Vishwar Randi": 2,
      "Cannon Soldier": 1,
      Sangan: 1,
      "Water Element": 3,
      "Mega Thunderball": 3,
      Prisman: 3,
      "Mystical Sheep #1": 3,
      "Djinn the Watcher of the Wind": 3,
      Alinsection: 3,
      "Muse-A": 3,
      "Twin-Headed Thunder Dragon": 1,
      "Millennium Shield": 1,
      "Fiend's Hand": 1,
      "Shadow Ghoul": 1,
      "Mooyan Curry": 1,
      Megamorph: 1,
      "Monster Reborn": 1,
      "Spellbinding Circle": 1,
      "Dark-Piercing Light": 1,
      "Bear Trap": 1,
      "Fake Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "KaibaCorp",
    payout: 0,
    ante: [],
  },
  {
    name: "Roland",
    deck: {
      Doron: 1,
      "Little Chimera": 1,
      "Milus Radiant": 1,
      "Sinister Serpent": 1,
      "Mask of Darkness": 1,
      "Abyss Flower": 1,
      "Muka Muka": 1,
      "Dharma Cannon": 1,
      "Morphing Jar": 1,
      "Frog the Jam": 1,
      "Penguin Soldier": 1,
      "Pot the Trick": 1,
      "The Last Warrior from Another Planet": 1,
      "Black Luster Soldier": 3,
      "Master of Dragon Soldier": 3,
      "Bolt Escargot": 1,
      "Saber Slasher": 1,
      "Witch's Apprentice": 1,
      "Blue-Eyes Ultimate Dragon": 3,
      "Gale Dogra": 1,
      "Queen's Double": 1,
      "White Dolphin": 1,
      Mechaleon: 1,
      Griggle: 1,
      "Zone Eater": 1,
      "Bone Mouse": 1,
      "Hiro's Shadow Scout": 1,
      "The Unhappy Maiden": 1,
      "Pot of Greed": 1,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "KaibaCorp",
    payout: 0,
    ante: [
      "Mask of Darkness",
      "Doron",
      "Dark Hole",
      "Mooyan Curry",
      "Saber Slasher",
      "Bolt Escargot",
      "Morinphen",
      "Gale Dogra",
      "Muka Muka",
      "Mechaleon",
      "Skelengel",
      "Dharma Cannon",
      "Frog the Jam",
      "Abyss Flower",
      "Morphing Jar",
      "Little Chimera",
      "Penguin Soldier",
      "Hiro's Shadow Scout",
      "Witch's Apprentice",
      "Pot of Greed",
      "The Last Warrior from Another Planet",
    ],
  },
  {
    name: "Mokuba",
    deck: {
      "Blue-Eyed Silver Zombie": 3,
      "Man-Eating Plant": 3,
      Skelengel: 3,
      "Monster Eye": 3,
      "Jinzo #7": 3,
      Krokodilus: 3,
      "Mechanical Spider": 3,
      "Mystic Lamp": 3,
      Leghul: 3,
      Bat: 3,
      "Witch's Apprentice": 1,
      "Darkness Approaches": 3,
      "Mooyan Curry": 2,
      "House of Adhesive Tape": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "KaibaCorp",
    payout: 0,
    ante: [
      "Man-Eating Plant",
      "Krokodilus",
      "Blue-Eyed Silver Zombie",
      "Mooyan Curry",
      "Mystic Lamp",
      "Skelengel",
      "Witch's Apprentice",
      "House of Adhesive Tape",
      "Darkness Approaches",
    ],
  },
  {
    name: "Puppeteer of Doom",
    deck: {
      "Megirus Light": 3,
      "Wicked Dragon with the Ersatz Head": 3,
      "Phantom Dewan": 3,
      "Two-Mouth Darkruler": 3,
      "Blue-Eyed Silver Zombie": 3,
      "Needle Ball": 3,
      "Vishwar Randi": 3,
      "Gate Deeg": 3,
      "Doll of Demise": 2,
      Ansatsu: 2,
      "Giant Mech-Soldier": 2,
      "The Last Warrior from Another Planet": 2,
      "Malevolent Nuzzler": 2,
      "Final Flame": 2,
      Yami: 2,
      Eatgaboon: 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 600,
    ante: [
      "The Last Warrior from Another Planet",
      "Final Flame",
      "Ansatsu",
      "Doll of Demise",
      "Yami",
      "Phantom Dewan",
      "Needle Ball",
      "Gate Deeg",
      "Giant Mech-Soldier",
      "Blue-Eyed Silver Zombie",
      "Two-Mouth Darkruler",
      "Vishwar Randi",
      "Megirus Light",
      "Wicked Dragon with the Ersatz Head",
      "Malevolent Nuzzler",
    ],
  },
  {
    name: "Jinzo",
    deck: {
      "Patrol Robo": 3,
      Prisman: 3,
      "Cyber Jar": 3,
      "Blast Juggler": 3,
      "White Magical Hat": 3,
      "Necrolancer the Timelord": 3,
      "Electric Snake": 3,
      "Turtle Raccoon": 3,
      "Injection Fairy Lily": 3,
      Jinzo: 1,
      "Crow Goblin": 1,
      "Psycho-Puppet": 1,
      "Hiro's Shadow Scout": 3,
      Sparks: 1,
      "Mooyan Curry": 3,
      Hinotama: 1,
      "Machine Conversion Factory": 1,
      "Heavy Storm": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "KaibaCorp",
    payout: 800,
    ante: [
      "Cyber Jar",
      "Satellite Cannon",
      "Injection Fairy Lily",
      "Hiro's Shadow Scout",
    ],
  },
  {
    name: "Johnny Steps",
    deck: {
      "Water Omotics": 1,
      "Karate Man": 3,
      "Lady of Faith": 2,
      Arlownay: 3,
      "Muse-A": 3,
      "Goddess of Whim": 3,
      Prisman: 3,
      "Sonic Maid": 3,
      "Rose Spectre of Dunn": 3,
      "Musician King": 3,
      "Witch of the Black Forest": 2,
      "Vishwar Randi": 3,
      "Spirit of the Harp": 1,
      Metalmorph: 3,
      "Goblin Fan": 1,
      "Bear Trap": 1,
      "Reverse Trap": 1,
      "Bad Reaction to Simochi": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Crump",
    deck: {
      "Kappa Avenger": 3,
      Doron: 3,
      "Toad Master": 3,
      "Penguin Soldier": 3,
      Akihiron: 3,
      "Bolt Penguin": 3,
      "Flying Penguin": 3,
      "Twin Long Rods #1": 3,
      "Penguin Torpedo": 3,
      "Penguin Knight": 3,
      Octoberser: 3,
      "Nightmare Penguin": 1,
      Umi: 3,
      "Power of Kaishin": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Strings",
    deck: {
      Monsturtle: 3,
      "Karate Man": 3,
      Akihiron: 3,
      "Barrel Rock": 3,
      "Kappa Avenger": 3,
      "Revival Jam": 3,
      "Humanoid Slime": 3,
      "Frog the Jam": 3,
      "Humanoid Worm Drake": 3,
      "Worm Drake": 3,
      "The Statue of Easter Island": 3,
      Prisman: 3,
      "Power of Kaishin": 2,
      "Invisible Wire": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Galapagos",
    payout: 1000,
    ante: [
      "Humanoid Slime",
      "Worm Drake",
      "Humanoid Worm Drake",
      "Revival Jam",
      "Jam Breeding Machine",
    ],
  },
  {
    name: "Umbra",
    deck: {
      "Nuvia the Wicked": 3,
      "Tao the Chanter": 3,
      Morinphen: 3,
      Togex: 3,
      "Blue-Eyed Silver Zombie": 3,
      "The Masked Beast": 1,
      "Mask of Darkness": 3,
      "Giant Germ": 3,
      "Shining Abyss": 3,
      "The Statue of Easter Island": 3,
      "Tripwire Beast": 3,
      "Masked Sorcerer": 3,
      "Malevolent Nuzzler": 1,
      "Bear Trap": 1,
      Eatgaboon: 1,
      "Darkness Approaches": 3,
    },
    lp: 8000,
    field: "Yami",
    location: "China",
    payout: 0,
    ante: ["Shining Abyss", "The Masked Beast", "Nuvia the Wicked"],
  },
  {
    name: "Lumis",
    deck: {
      "Mystic Tomato": 3,
      "Stuffed Animal": 3,
      Lisark: 3,
      "Masked Sorcerer": 3,
      "Masked Beast Des Gardius": 1,
      Takuhee: 3,
      "Frenzied Panda": 3,
      "Grand Tiki Elder": 3,
      "Magical Ghost": 3,
      "Mushroom Man #2": 3,
      "Mask of Shine & Dark": 3,
      "Melchid the Four-Face Beast": 3,
      "Malevolent Nuzzler": 1,
      "Dark-Piercing Light": 2,
      "Invisible Wire": 1,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "China",
    payout: 900,
    ante: [
      "Grand Tiki Elder",
      "Melchid the Four-Face Beast",
      "Masked Beast Des Gardius",
    ],
  },
  {
    name: "Arkana",
    deck: {
      "Big Eye": 3,
      "Ancient Elf": 3,
      "Legion the Fiend Jester": 3,
      "Fairy's Gift": 3,
      "Mystic Clown": 3,
      "Witty Phantom": 3,
      "Doll of Demise": 3,
      "Dark Magician": 3,
      "The Stern Mystic": 3,
      "Black Pendant": 3,
      "Book of Secret Arts": 3,
      "Beckon to Darkness": 2,
      Yami: 2,
      "Monster Reborn": 1,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Domino Pier",
    payout: 800,
    ante: [
      "The Stern Mystic",
      "Dark Magician",
      "Witty Phantom",
      "Big Eye",
      "Mystic Clown",
      "Black Pendant",
      "Book of Secret Arts",
      "Yami",
      "Fairy's Gift",
      "Ancient Elf",
      "Monster Reborn",
      "Bear Trap",
      "Beckon to Darkness",
      "Legion the Fiend Jester",
      "Doll of Demise",
    ],
  },
  {
    name: "Bandit Keith",
    deck: {
      "The Winged Dragon of Ra (Sphere Mode)": 1,
      Metalzoa: 1,
      "Zera The Mant": 1,
      Zoa: 1,
      "Barrel Dragon": 2,
      Seiyaryu: 1,
      "Launcher Spider": 1,
      "Slot Machine": 2,
      "Machine King": 1,
      "Pendulum Machine": 1,
      "Space Megatron": 1,
      Mechanicalchaser: 1,
      "Whiptail Crow": 2,
      "Ryu-Kishin Powered": 3,
      "Blast Sphere": 3,
      "Ryu-Kishin": 2,
      "7 Completed": 3,
      "Brain Control": 1,
      "Change of Heart": 1,
      Metalmorph: 1,
      "Machine Conversion Factory": 3,
      "Monster Reborn": 1,
      "Pot of Greed": 1,
      "Tremendous Fire": 2,
      "Invisible Wire": 3,
    },
    lp: 8000,
    field: "Yami",
    location: "Egypt Exhibition",
    payout: 0,
    ante: [
      "Seiyaryu",
      "Zera The Mant",
      "Pendulum Machine",
      "Launcher Spider",
      "Zoa",
      "Metalzoa",
      "Machine King",
      "Mechanicalchaser",
      "Slot Machine",
      "Blast Sphere",
      "Barrel Dragon",
    ],
  },
  {
    name: "Odion",
    deck: {
      "Mystical Beast Serket": 1,
      "Talons of Shurilane": 3,
      "Embodiment of Apophis": 3,
      "Last Tusk Mammoth": 1,
      "Giant Rex": 1,
      Keldo: 3,
      "Tri-Horned Dragon": 3,
      "Swords of Revealing Light": 3,
      Raigeki: 2,
      "Heavy Storm": 1,
      "Final Destiny": 1,
      "Monster Reborn": 3,
      "Dark Hole": 1,
      "Harpie's Feather Duster": 1,
      "Gravedigger Ghoul": 1,
      "The Inexperienced Spy": 1,
      "Beckon to Darkness": 1,
      "Restructer Revolution": 1,
      "Stop Defense": 1,
      "Acid Trap Hole": 1,
      "Widespread Ruin": 1,
      "Torrential Tribute": 1,
      "Infinite Dismissal": 1,
      "Bear Trap": 1,
      "Amazon Archers": 1,
      "Anti Raigeki": 1,
      "Invisible Wire": 1,
      "Dragon Capture Jar": 1,
      "House of Adhesive Tape": 1,
      Eatgaboon: 1,
      "Goblin Fan": 1,
      "Bad Reaction to Simochi": 1,
      "Reverse Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Egypt",
    payout: 0,
    ante: [
      "Keldo",
      "Final Destiny",
      "Heavy Storm",
      "Stop Defense",
      "Dragon Capture Jar",
      "Dark Hole",
      "Raigeki",
      "Swords of Revealing Light",
      "Dark-Piercing Light",
      "Giant Rex",
      "Embodiment of Apophis",
      "Last Tusk Mammoth",
      "Monster Reborn",
      "Gravedigger Ghoul",
      "Amazon Archers",
      "Cursebreaker",
      "Mystical Beast Serket",
      "Harpie's Feather Duster",
      "Bear Trap",
      "Invisible Wire",
      "Acid Trap Hole",
      "Widespread Ruin",
      "Bad Reaction to Simochi",
      "Reverse Trap",
      "Torrential Tribute",
      "Beckon to Darkness",
      "Infinite Dismissal",
      "Tri-Horned Dragon",
      "Anti Raigeki",
      "Restructer Revolution",
      "The Inexperienced Spy",
      "Talons of Shurilane",
    ],
  },
  {
    name: "Marik Ishtar",
    deck: {
      "Cyber-Tech Alligator": 1,
      "Humanoid Worm Drake": 1,
      "Humanoid Slime": 3,
      "Worm Drake": 3,
      "Executor - Makyura": 3,
      "Revival Jam": 3,
      Newdoria: 3,
      "Viser Des": 3,
      "Darkfire Soldier #1": 2,
      "Talons of Shurilane": 1,
      "Byser Shock": 1,
      "Des Volstgalph": 1,
      "Swords of Revealing Light": 3,
      "Monster Reborn": 3,
      "Soul of the Pure": 2,
      "Harpie's Feather Duster": 1,
      Raigeki: 2,
      "Pot of Greed": 1,
      "Spellbinding Circle": 1,
      "Widespread Ruin": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Egypt",
    payout: 0,
    ante: [
      "Summoned Skull",
      "Judge Man",
      "Dark Zebra",
      "Amphibian Beast",
      "Darkfire Soldier #1",
      "Raigeki",
      "Soul of the Pure",
      "Swords of Revealing Light",
      "Spellbinding Circle",
      "Mechanicalchaser",
      "Harpie's Brother",
      "Giant Red Seasnake",
      "Kaminari Attack",
      "The Immortal of Thunder",
      "Garvas",
      "Asura Priest",
      "Monster Reborn",
      "Des Volstgalph",
      "Exarion Universe",
      "Widespread Ruin",
      "Cosmo Queen",
      "Meteor B. Dragon",
      "Firewing Pegasus",
      "Neo the Magic Swordsman",
      "Beast of Gilfer",
      "Dunames Dark Witch",
      "Cyber-Tech Alligator",
      "Talons of Shurilane",
    ],
  },
  {
    name: "Paradox",
    deck: {
      "Battle Ox": 3,
      Keldo: 3,
      "Darkfire Soldier #1": 3,
      "Zera The Mant": 1,
      "Millennium Golem": 3,
      "Guardian of the Throne Room": 3,
      "Fire Kraken": 3,
      "Monster Reborn": 1,
      "High Tide Gyojin": 3,
      "Whiptail Crow": 3,
      "Torrential Tribute": 1,
      Sengenjin: 1,
      "Tri-Horned Dragon": 1,
      "Skull Knight": 1,
      "Cosmo Queen": 1,
      "Crab Turtle": 1,
      "Garma Sword": 1,
      "Reflect Bounder": 3,
      "Neo the Magic Swordsman": 3,
      "Change of Heart": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "China",
    payout: 0,
    ante: [
      "Battle Ox",
      "Keldo",
      "Darkfire Soldier #1",
      "Zera The Mant",
      "Millennium Golem",
      "Guardian of the Throne Room",
      "Fire Kraken",
      "Monster Reborn",
      "High Tide Gyojin",
      "Whiptail Crow",
      "Torrential Tribute",
      "Sengenjin",
      "Tri-Horned Dragon",
      "Skull Knight",
      "Cosmo Queen",
      "Crab Turtle",
      "Garma Sword",
      "Reflect Bounder",
      "Neo the Magic Swordsman",
      "Change of Heart",
    ],
  },
  {
    name: "Dark Mokuba",
    deck: {
      "Gemini Elf": 2,
      "Witty Phantom": 2,
      "Man-Eating Treasure Chest": 2,
      "Darkfire Soldier #1": 2,
      "Skull Red Bird": 2,
      Mechanicalchaser: 2,
      "Dark Zebra": 2,
      "Yamata Dragon": 2,
      "Dragon Seeker": 1,
      "High Tide Gyojin": 3,
      "Talons of Shurilane": 1,
      "Aqua Dragon": 1,
      "Thunder Dragon": 1,
      "Dark Chimera": 1,
      "The Immortal of Thunder": 2,
      "B. Dragon Jungle King": 1,
      "Judge Man": 1,
      "Sky Dragon": 1,
      "Twin-Headed Fire Dragon": 1,
      "Tremendous Fire": 2,
      "Swords of Revealing Light": 3,
      "Dragon Treasure": 2,
      "Gravedigger Ghoul": 1,
      "Widespread Ruin": 2,
    },
    lp: 10000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Judge Man",
      "Dark Chimera",
      "Witty Phantom",
      "Cyber Jar",
      "Twin-Headed Fire Dragon",
      "Darkfire Soldier #1",
      "Kanan the Swordmistress",
      "Mechanicalchaser",
      "Sky Dragon",
      "Thunder Dragon",
      "Aqua Dragon",
      "The Immortal of Thunder",
      "Skull Red Bird",
      "Dragon Seeker",
      "Gemini Elf",
      "B. Dragon Jungle King",
      "High Tide Gyojin",
      "Yamata Dragon",
      "Des Volstgalph",
      "Man-Eating Treasure Chest",
      "Talons of Shurilane",
    ],
  },
  {
    name: "Seto Kaiba",
    deck: {
      "Ancient Lamp": 1,
      "Blade Knight": 3,
      "Des Feral Imp": 2,
      "Steel Ogre Grotto #2": 1,
      Swordstalker: 1,
      "Invitation to a Dark Sleep": 1,
      "Blue-Eyes White Dragon": 3,
      "Z-Metal Tank": 3,
      "Vorse Raider": 3,
      "Lord of D.": 1,
      "X-Head Cannon": 3,
      "Y-Dragon Head": 3,
      "Blue-Eyes Ultimate Dragon": 1,
      "Obelisk the Tormentor": 1,
      Megamorph: 3,
      "Crush Card": 2,
      "Heavy Storm": 1,
      "Shadow Spell": 2,
      "Dark Energy": 3,
      "Swords of Revealing Light": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Pegasus Castle",
    payout: 0,
    ante: ["Blue-Eyes White Dragon"],
  },
  {
    name: "Sol Chevalsky",
    deck: {
      "Thousand-Eyes Restrict": 3,
      Relinquished: 2,
      "Dark-Eyes Illusionist": 1,
      "Toon Dark Magician Girl": 1,
      "Toon Summoned Skull": 1,
      "Rogue Doll": 3,
      "Dark Rabbit": 1,
      "Toon Alligator": 1,
      "Toon Mermaid": 3,
      "Jigen Bakudan": 1,
      "Oscillo Hero #2": 1,
      Bickuribox: 1,
      "Parrot Dragon": 1,
      "Blue-Eyes Toon Dragon": 1,
      "Dragon Piper": 1,
      "Ryu-Ran": 1,
      "Illusionist Faceless Mage": 1,
      "Manga Ryu-Ran": 1,
      "Red Archery Girl": 3,
      "Swords of Revealing Light": 3,
      "Tremendous Fire": 2,
      "Harpie's Feather Duster": 3,
      "Change of Heart": 2,
      "Torrential Tribute": 1,
      "Dragon Capture Jar": 1,
    },
    lp: 20000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Dragon Capture Jar",
      "Dark-Eyes Illusionist",
      "Relinquished",
      "Jigen Bakudan",
      "Thousand-Eyes Idol",
      "Thousand-Eyes Restrict",
      "Black Illusion Ritual",
    ],
  },
  {
    name: "Reshef",
    deck: {
      "Vorse Raider": 3,
      Mechanicalchaser: 3,
      "Zombyra the Dark": 2,
      "Nuvia the Wicked": 3,
      "Castle of Dark Illusions": 3,
      "Flash Assailant": 3,
      "Slifer the Sky Dragon": 1,
      "Obelisk the Tormentor": 1,
      "The Winged Dragon of Ra (Battle Mode)": 1,
      "King of Yamimakai": 1,
      "Swords of Revealing Light": 3,
      "Pot of Greed": 3,
      "Harpie's Feather Duster": 2,
      "Monster Reborn": 1,
      Megamorph: 2,
      Raigeki: 1,
      "Change of Heart": 3,
      "Tremendous Fire": 1,
      "Torrential Tribute": 3,
    },
    lp: 40000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Raigeki",
      "Megamorph",
      "Torrential Tribute",
      "Monster Reborn",
      "Brain Control",
      "Mechanicalchaser",
      "Vorse Raider",
      "Exarion Universe",
      "Swords of Revealing Light",
      "Flash Assailant",
      "Nuvia the Wicked",
      "King of Yamimakai",
      "Zombyra the Dark",
      "Castle of Dark Illusions",
      "Change of Heart",
    ],
  },
  {
    name: "Duke Devlin 1",
    deck: {
      "Winged Cleaver": 3,
      "Hourglass of Life": 3,
      "The Bewitching Phantom Thief": 3,
      Firegrass: 3,
      "Serpent Marauder": 3,
      Kaminarikozou: 3,
      "Morphing Jar": 3,
      "Wings of Wicked Flame": 3,
      Meotoko: 3,
      "Phantom Dewan": 3,
      "Wicked Mirror": 3,
      "Boo Koo": 3,
      "Restructer Revolution": 1,
      "The Inexperienced Spy": 1,
      "Heavy Storm": 1,
      "Fake Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Game Shop",
    payout: 0,
    ante: [],
  },
  {
    name: "Duke Devlin 2",
    deck: {
      "Hourglass of Life": 3,
      "The Statue of Easter Island": 2,
      "Orgoth the Relentless": 1,
      Gradius: 3,
      "Weather Control": 3,
      "Fiend's Hand": 3,
      "Spellbinding Circle": 3,
      "Beckon to Darkness": 3,
      "Stop Defense": 3,
      "Cyclon Laser": 3,
      "Machine Conversion Factory": 3,
      "Darkness Approaches": 3,
      Raigeki: 1,
      "Soul of the Pure": 3,
      "Invisible Wire": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Game Shop",
    payout: 0,
    ante: [],
  },
  {
    name: "PaniK 1",
    deck: {
      "Curtain of the Dark Ones": 3,
      Meotoko: 3,
      "Cyber-Stein": 3,
      "The Melting Red Shadow": 3,
      "Fiend's Hand": 3,
      "Phantom Dewan": 3,
      "Mystic Lamp": 3,
      "Saber Slasher": 3,
      "Succubus Knight": 3,
      "Monster Eye": 3,
      "Mooyan Curry": 2,
      "Violet Crystal": 1,
      "Black Pendant": 1,
      "Malevolent Nuzzler": 1,
      Yami: 3,
      Eatgaboon: 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Duel Express",
    payout: 100,
    ante: [
      "Mooyan Curry",
      "Saber Slasher",
      "Succubus Knight",
      "Curtain of the Dark Ones",
      "Fiend's Hand",
      "Yami",
      "The Melting Red Shadow",
      "Phantom Dewan",
      "Meotoko",
      "Cyber-Stein",
      "Black Pendant",
      "Malevolent Nuzzler",
      "Violet Crystal",
    ],
  },
  {
    name: "PaniK 2",
    deck: {
      "Dark Assailant": 3,
      Barox: 2,
      "King of Yamimakai": 2,
      "Baron of the Fiend Sword": 3,
      "Castle of Dark Illusions": 1,
      "One Who Hunts Souls": 3,
      "Reaper of the Cards": 2,
      "Metal Guardian": 2,
      "Dark Chimera": 2,
      "Fiend Sword": 3,
      "Dark King of the Abyss": 3,
      "Soul of the Pure": 2,
      "Swords of Revealing Light": 3,
      "Dark Energy": 2,
      "Tremendous Fire": 2,
      Yami: 3,
      "Invisible Wire": 2,
    },
    lp: 15000,
    field: "Yami",
    location: "Pegasus Island",
    payout: 0,
    ante: [
      "Reaper of the Cards",
      "Barox",
      "Dark Chimera",
      "King of Yamimakai",
      "Metal Guardian",
      "Castle of Dark Illusions",
    ],
  },
  {
    name: "Mimic of Doom 1",
    deck: {
      "Invader from Another Dimension": 3,
      "Big Eye": 3,
      "Frenzied Panda": 3,
      Mechaleon: 3,
      "Three-Legged Zombies": 3,
      "Kappa Avenger": 3,
      "Giant Germ": 3,
      Togex: 1,
      Genin: 3,
      "Dream Clown": 3,
      Snakeyashi: 3,
      "Mystical Sheep #1": 3,
      "Talons of Shurilane": 1,
      "Soul Hunter": 1,
      "Final Destiny": 1,
      Ookazi: 1,
      "Mooyan Curry": 1,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Yami",
    location: "Galapagos",
    payout: 1200,
    ante: [
      "Dream Clown",
      "Big Eye",
      "Giant Germ",
      "Final Destiny",
      "Frenzied Panda",
      "Mooyan Curry",
      "Ookazi",
      "Three-Legged Zombies",
      "Kappa Avenger",
      "Soul Hunter",
      "Togex",
      "Invader from Another Dimension",
      "Snakeyashi",
      "Mystical Sheep #1",
      "Bear Trap",
      "Talons of Shurilane",
    ],
  },
  {
    name: "Mimic of Doom 2",
    deck: {
      "Dream Clown": 3,
      "Sonic Maid": 3,
      "Rogue Doll": 3,
      "Talons of Shurilane": 3,
      "Dimensional Warrior": 3,
      "Man-Eating Treasure Chest": 3,
      "Musician King": 2,
      "Wall of Illusion": 3,
      "The Judgement Hand": 3,
      "Soul of the Pure": 2,
      "Tremendous Fire": 2,
      Megamorph: 2,
      "Swords of Revealing Light": 3,
      Yami: 3,
      "Invisible Wire": 2,
    },
    lp: 15000,
    field: "Yami",
    location: "Pegasus Island",
    payout: 0,
    ante: [
      "Rogue Doll",
      "Dream Clown",
      "The Judgement Hand",
      "Dimensional Warrior",
      "Holograh",
      "Sonic Maid",
      "Musician King",
      "Wall of Illusion",
      "Man-Eating Treasure Chest",
      "Talons of Shurilane",
    ],
  },
  {
    name: "Para 1",
    deck: {
      "Frenzied Panda": 3,
      "Bolt Penguin": 3,
      "Karate Man": 3,
      "Monster Tamer": 2,
      "Barrel Lily": 3,
      Prisman: 3,
      "Nightmare Scorpion": 3,
      "Electric Lizard": 3,
      Alinsection: 3,
      "Electric Snake": 3,
      "Shadow Ghoul": 2,
      "Wall Shadow": 1,
      "Labyrinth Wall": 1,
      "Sanga of the Thunder": 1,
      Cursebreaker: 3,
      "Magical Labyrinth": 3,
    },
    lp: 8000,
    field: "Yami",
    location: "China",
    payout: 300,
    ante: [
      "Magical Labyrinth",
      "Cursebreaker",
      "Shadow Ghoul",
      "Monster Tamer",
      "Wall Shadow",
      "Labyrinth Wall",
    ],
  },
  {
    name: "Para 2",
    deck: {
      "Jirai Gumo": 2,
      "Karate Man": 3,
      "Frenzied Panda": 3,
      "Monster Tamer": 2,
      "Bolt Penguin": 3,
      Kazejin: 1,
      Suijin: 1,
      "Shadow Ghoul": 2,
      Prisman: 3,
      "Dungeon Worm": 2,
      "Labyrinth Wall": 1,
      "Barrel Lily": 3,
      "Electric Snake": 3,
      "Wall Shadow": 1,
      "Sanga of the Thunder": 1,
      "Gate Guardian": 1,
      "Labyrinth Tank": 1,
      Megamorph: 1,
      Cursebreaker: 2,
      "Magical Labyrinth": 2,
      Eatgaboon: 2,
    },
    lp: 8000,
    field: "Yami",
    location: "China",
    payout: 800,
    ante: ["Kazejin", "Suijin", "Sanga of the Thunder"],
  },
  {
    name: "Para 3",
    deck: {
      "Mystic Tomato": 3,
      "Sanga of the Thunder": 1,
      "Tripwire Beast": 3,
      "Monster Tamer": 1,
      "The Immortal of Thunder": 3,
      "Destroyer Golem": 3,
      Kojikocy: 3,
      Kazejin: 1,
      "Labyrinth Tank": 1,
      "Wall Shadow": 1,
      "Jirai Gumo": 3,
      "Shadow Ghoul": 1,
      "Dungeon Worm": 1,
      "Labyrinth Wall": 3,
      Suijin: 1,
      "Gate Guardian": 1,
      "Tremendous Fire": 1,
      "Swords of Revealing Light": 3,
      Cursebreaker: 2,
      "Magical Labyrinth": 2,
      "Invisible Wire": 2,
    },
    lp: 15000,
    field: "Yami",
    location: "Dungeon",
    payout: 0,
    ante: ["Gate Guardian"],
  },
  {
    name: "Dox 1",
    deck: {
      "Jirai Gumo": 2,
      "Dragoness the Wicked Knight": 3,
      "Aqua Snake": 3,
      "Monster Tamer": 2,
      "Charubin the Fire Knight": 3,
      Kazejin: 1,
      Suijin: 1,
      "Shadow Ghoul": 2,
      "Princess of Tsurugi": 3,
      "Dungeon Worm": 2,
      "Labyrinth Wall": 1,
      "Root Water": 3,
      "Blast Juggler": 3,
      "Wall Shadow": 1,
      "Sanga of the Thunder": 1,
      "Gate Guardian": 1,
      "Labyrinth Tank": 1,
      Megamorph: 1,
      Cursebreaker: 2,
      "Magical Labyrinth": 2,
      Eatgaboon: 2,
    },
    lp: 8000,
    field: "Yami",
    location: "China",
    payout: 0,
    ante: ["Dungeon Worm"],
  },
  {
    name: "Dox 2",
    deck: {
      "Skull Red Bird": 3,
      "Sanga of the Thunder": 1,
      "UFO Turtle": 3,
      "Monster Tamer": 1,
      "Darkfire Dragon": 3,
      Lisark: 3,
      "Crazy Fish": 3,
      Kazejin: 1,
      "Labyrinth Tank": 1,
      "Wall Shadow": 1,
      "Jirai Gumo": 3,
      "Shadow Ghoul": 1,
      "Dungeon Worm": 1,
      Suijin: 1,
      "Gate Guardian": 1,
      "Tremendous Fire": 1,
      "Swords of Revealing Light": 3,
      "Labyrinth Wall": 3,
      Cursebreaker: 2,
      "Magical Labyrinth": 2,
      "Invisible Wire": 2,
    },
    lp: 15000,
    field: "Yami",
    location: "Dungeon",
    payout: 0,
    ante: ["Gate Guardian"],
  },
  {
    name: "Jean-Claude Magnum 1",
    deck: {
      "The Bewitching Phantom Thief": 3,
      "Curtain of the Dark Ones": 3,
      "Armed Ninja": 3,
      "Lucky Trinket": 3,
      Genin: 3,
      "Mystic Lamp": 3,
      "Phantom Dewan": 3,
      "Queen's Double": 3,
      "Legendary Sword": 1,
      "Monster Egg": 3,
      "Boo Koo": 3,
      "Djinn the Watcher of the Wind": 3,
      "Sword of Dark Destruction": 1,
      "Black Pendant": 1,
      Eatgaboon: 1,
      "House of Adhesive Tape": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 200,
    ante: [
      "Curtain of the Dark Ones",
      "Djinn the Watcher of the Wind",
      "The Bewitching Phantom Thief",
      "Monster Egg",
      "Phantom Dewan",
      "Lucky Trinket",
      "Genin",
      "Legendary Sword",
      "Sword of Dark Destruction",
      "Black Pendant",
      "Mystic Lamp",
      "Armed Ninja",
      "Boo Koo",
      "Queen's Double",
      "House of Adhesive Tape",
      "Eatgaboon",
    ],
  },
  {
    name: "Jean-Claude Magnum 2",
    deck: {
      "Vishwar Randi": 3,
      "Princess of Tsurugi": 3,
      Doron: 3,
      "Karate Man": 3,
      "Hungry Burger": 3,
      "Water Element": 3,
      "Goddess of Whim": 3,
      "Armed Ninja": 3,
      "Muse-A": 3,
      Arlownay: 3,
      "Kagemusha of the Blue Flame": 3,
      "Queen's Double": 3,
      "Bear Trap": 1,
      "Goblin Fan": 1,
      "Reverse Trap": 1,
      "Bad Reaction to Simochi": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Domino Pier",
    payout: 0,
    ante: [
      "Arlownay",
      "Karate Man",
      "Doron",
      "Kagemusha of the Blue Flame",
      "Vishwar Randi",
      "Water Element",
      "Princess of Tsurugi",
      "Goddess of Whim",
      "Armed Ninja",
      "Muse-A",
      "Queen's Double",
      "Bear Trap",
      "Goblin Fan",
      "Bad Reaction to Simochi",
      "Reverse Trap",
      "Hungry Burger",
    ],
  },
  {
    name: "Rare Hunter 1",
    deck: {
      "Three-Headed Geedo": 3,
      "Shovel Crusher": 3,
      "Goddess of Whim": 3,
      "Hard Armor": 3,
      "Stone Statue of the Aztecs": 3,
      "Hannibal Necromancer": 3,
      "Exodia the Forbidden One": 3,
      "Left Arm of the Forbidden One": 3,
      "Right Arm of the Forbidden One": 3,
      "Left Leg of the Forbidden One": 3,
      "Right Leg of the Forbidden One": 3,
      "Gear Golem the Moving Fortress": 3,
      "Wood Clown": 3,
      "Pot of Greed": 1,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 600,
    ante: [
      "Right Leg of the Forbidden One",
      "Left Leg of the Forbidden One",
      "Right Arm of the Forbidden One",
      "Left Arm of the Forbidden One",
      "Exodia the Forbidden One",
    ],
  },
  {
    name: "Rare Hunter 2",
    deck: {
      "Exodia the Forbidden One": 3,
      "Left Arm of the Forbidden One": 3,
      "Right Arm of the Forbidden One": 3,
      "Left Leg of the Forbidden One": 3,
      "Right Leg of the Forbidden One": 3,
      Skelengel: 3,
      "Goddess of Whim": 3,
      "Mysterious Puppeteer": 3,
      Yaranzo: 3,
      "Three-Headed Geedo": 3,
      "Stone Statue of the Aztecs": 3,
      "Gear Golem the Moving Fortress": 3,
      "Hannibal Necromancer": 3,
      "Pot of Greed": 1,
    },
    lp: 8000,
    field: "Yami",
    location: "KaibaCorp",
    payout: 800,
    ante: [
      "Hannibal Necromancer",
      "Three-Headed Geedo",
      "Stone Statue of the Aztecs",
      "Gear Golem the Moving Fortress",
    ],
  },
  {
    name: "Bonz 1",
    deck: {
      "Wicked Mirror": 3,
      "Fire Reaper": 3,
      "Dragon Zombie": 1,
      "Clown Zombie": 1,
      "Hiro's Shadow Scout": 3,
      "Wood Remains": 3,
      "Needle Ball": 3,
      "The Shadow Who Controls the Dark": 3,
      "Pumpking the King of Ghosts": 1,
      "Blue-Eyed Silver Zombie": 3,
      "Phantom Ghost": 3,
      "Armored Zombie": 1,
      "Crass Clown": 1,
      Zanki: 1,
      "Crawling Dragon": 1,
      Sparks: 2,
      "Violet Crystal": 2,
      "Gravedigger Ghoul": 3,
      Wasteland: 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Italy",
    payout: 400,
    ante: [
      "The Bewitching Phantom Thief",
      "The Melting Red Shadow",
      "Candle of Fate",
      "Mystic Lamp",
      "Kaiser Dragon",
      "Korogashi",
      "Boo Koo",
      "Hane-Hane",
      "Frog the Jam",
      "Morphing Jar",
      "Lightning Conger",
      "Meteor Dragon",
      "Meteor B. Dragon",
    ],
  },
  {
    name: "Bonz 2",
    deck: {
      "Dragon Zombie": 3,
      Skelgon: 2,
      "Temple of Skulls": 3,
      "The Snake Hair": 3,
      "Clown Zombie": 3,
      "Fiend's Hand": 3,
      "Wood Remains": 3,
      "Armored Zombie": 3,
      "Flame Ghost": 3,
      "Pumpking the King of Ghosts": 1,
      "Great Mammoth of Goldfine": 1,
      Wasteland: 3,
      "Gravedigger Ghoul": 3,
      "Violet Crystal": 3,
      "Bear Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "The Snake Hair",
      "Armored Zombie",
      "Dragon Zombie",
      "Clown Zombie",
      "Fiend's Hand",
      "Temple of Skulls",
      "Flame Ghost",
      "Wood Remains",
      "Violet Crystal",
      "Wasteland",
      "Skelgon",
      "Great Mammoth of Goldfine",
      "Berserk Dragon",
      "Bear Trap",
      "Dokurorider",
    ],
  },
  {
    name: "Bonz 3",
    deck: {
      "Great Mammoth of Goldfine": 2,
      "Ancient Lamp": 1,
      "Baron of the Fiend Sword": 2,
      "Man-Eating Treasure Chest": 2,
      "Dragon Zombie": 3,
      "Fiend's Hand": 3,
      "The Snake Hair": 3,
      "Clown Zombie": 3,
      "Armored Zombie": 3,
      "Swords of Revealing Light": 2,
      "Pumpking the King of Ghosts": 1,
      "Violet Crystal": 3,
      "Monster Reborn": 3,
      "Gravedigger Ghoul": 3,
      Wasteland: 2,
      "Acid Trap Hole": 2,
      Dokurorider: 2,
    },
    lp: 8000,
    field: "Wasteland",
    location: "Italy",
    payout: 0,
    ante: [
      "The Snake Hair",
      "Armored Zombie",
      "Dragon Zombie",
      "Clown Zombie",
      "Baron of the Fiend Sword",
      "Shining Abyss",
      "Violet Crystal",
      "Wasteland",
      "Grand Tiki Elder",
      "The Masked Beast",
      "Ancient Lamp",
      "Great Mammoth of Goldfine",
      "Vampire Lord",
      "Berserk Dragon",
      "Melchid the Four-Face Beast",
      "Masked Beast Des Gardius",
      "Acid Trap Hole",
      "Dokurorider",
      "Man-Eating Treasure Chest",
    ],
  },
  {
    name: "Bonz (Imposter)",
    deck: {
      "Baron of the Fiend Sword": 3,
      "Man-Eating Treasure Chest": 3,
      "Whiptail Crow": 3,
      "Big Eye": 3,
      "Magical Ghost": 3,
      "Dark Assailant": 3,
      Skelgon: 3,
      Barox: 2,
      "Flame Ghost": 3,
      Yaranzo: 3,
      "Mystic Clown": 3,
      "Swords of Revealing Light": 1,
      "Violet Crystal": 2,
      Wasteland: 1,
      "Goblin's Secret Remedy": 1,
      Ookazi: 1,
      "Acid Trap Hole": 2,
    },
    lp: 10000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Barox",
      "Baron of the Fiend Sword",
      "Big Eye",
      "Flame Ghost",
      "Dark Assailant",
      "Mystic Clown",
      "Yaranzo",
      "Magical Ghost",
      "Skelgon",
      "Whiptail Crow",
      "Man-Eating Treasure Chest",
    ],
  },
  {
    name: "Rex 1",
    deck: {
      "Lesser Dragon": 3,
      "Stone D.": 1,
      Tomozaurus: 3,
      "One-Eyed Shield Dragon": 3,
      Trakadon: 2,
      "Fairy Dragon": 3,
      "Wicked Dragon with the Ersatz Head": 3,
      "Two-Headed King Rex": 1,
      "Petit Dragon": 3,
      "Yamatano Dragon Scroll": 3,
      "Two-Mouth Darkruler": 3,
      "Sword Arm of Dragon": 3,
      Megazowler: 3,
      "Little D": 3,
      "Raise Body Heat": 3,
    },
    lp: 8000,
    field: "Wasteland",
    location: "Canada",
    payout: 800,
    ante: [
      "Sword Arm of Dragon",
      "Megazowler",
      "Yamatano Dragon Scroll",
      "Two-Mouth Darkruler",
      "One-Eyed Shield Dragon",
      "Wicked Dragon with the Ersatz Head",
      "Raise Body Heat",
      "Stone D.",
      "Lesser Dragon",
      "Trakadon",
      "Little D",
      "Fairy Dragon",
    ],
  },
  {
    name: "Rex 2",
    deck: {
      "Sword Arm of Dragon": 3,
      Uraby: 2,
      Yormungarde: 3,
      Trakadon: 3,
      Pragtical: 2,
      "Darkfire Dragon": 2,
      "Minomushi Warrior": 2,
      "Fairy Dragon": 3,
      "Little D": 3,
      "Destroyer Golem": 2,
      "Two-Headed King Rex": 1,
      "Lesser Dragon": 3,
      Megazowler: 3,
      "Serpent Night Dragon": 2,
      Wasteland: 3,
      "Raise Body Heat": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Sword Arm of Dragon",
      "Megazowler",
      "Uraby",
      "Darkfire Dragon",
      "Raise Body Heat",
      "Wasteland",
      "Destroyer Golem",
      "Minomushi Warrior",
      "Pragtical",
      "Lesser Dragon",
      "Yormungarde",
      "Trakadon",
      "Little D",
      "Fairy Dragon",
      "Serpent Night Dragon",
    ],
  },
  {
    name: "Rex 3",
    deck: {
      "Sword Arm of Dragon": 2,
      "Crawling Dragon #2": 3,
      "Blackland Fire Dragon": 3,
      "Koumori Dragon": 3,
      "Darkfire Dragon": 3,
      "Serpent Night Dragon": 3,
      Uraby: 3,
      "Two-Headed King Rex": 1,
      "Bracchio-raidus": 1,
      Megazowler: 1,
      Pragtical: 1,
      "Swords of Revealing Light": 2,
      Wasteland: 2,
      Megamorph: 3,
      "Pot of Greed": 2,
      "Harpie's Feather Duster": 2,
      "Raise Body Heat": 3,
      "Acid Trap Hole": 2,
    },
    lp: 8000,
    field: "Wasteland",
    location: "Canada",
    payout: 0,
    ante: [
      "Blackland Fire Dragon",
      "Sword Arm of Dragon",
      "Koumori Dragon",
      "Megazowler",
      "Uraby",
      "Crawling Dragon #2",
      "Darkfire Dragon",
      "Raise Body Heat",
      "Wasteland",
      "Gamma The Magnet Warrior",
      "Pragtical",
      "Bracchio-raidus",
      "Acid Trap Hole",
      "Valkyrion the Magna Warrior",
      "Serpent Night Dragon",
      "Meteor Dragon",
      "Meteor B. Dragon",
      "Alpha The Magnet Warrior",
      "Beta The Magnet Warrior",
    ],
  },
  {
    name: "Rex (Imposter)",
    deck: {
      "Darkfire Dragon": 3,
      "The Statue of Easter Island": 3,
      "Bracchio-raidus": 3,
      "Barrel Rock": 3,
      Trakadon: 3,
      "Sky Dragon": 3,
      "Little D": 3,
      "Fairy Dragon": 3,
      "Minomushi Warrior": 3,
      "Destroyer Golem": 3,
      "Lesser Dragon": 2,
      "Swords of Revealing Light": 1,
      "Raise Body Heat": 2,
      Wasteland: 1,
      "Goblin's Secret Remedy": 1,
      Ookazi: 1,
      "Acid Trap Hole": 2,
    },
    lp: 10000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Darkfire Dragon",
      "Sky Dragon",
      "Destroyer Golem",
      "Barrel Rock",
      "Minomushi Warrior",
      "Bracchio-raidus",
      "The Statue of Easter Island",
      "Lesser Dragon",
      "Trakadon",
      "Little D",
      "Fairy Dragon",
    ],
  },
  {
    name: "Weevil 1",
    deck: {
      "Parasite Paracide": 1,
      "Spiked Snail": 2,
      "Drill Bug": 2,
      Gokibore: 2,
      "Skull-Mark Lady Bug": 1,
      "Giant Flea": 1,
      "Soldier Ari": 2,
      Minar: 2,
      "Larvae Moth": 3,
      "Great Moth": 1,
      "Basic Insect": 3,
      "Killer Needle": 2,
      "Hercules Beetle": 2,
      "Insect Queen": 1,
      "Perfectly Ultimate Great Moth": 1,
      "Big Insect": 2,
      "Flying Kamakiri #1": 1,
      "Nightmare Scorpion": 2,
      "Pinch Hopper": 1,
      "Insect Armor with Laser Cannon": 2,
      "Eradicating Aerosol": 1,
      Forest: 1,
      "Laser Cannon Armor": 2,
      "Infinite Dismissal": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Weevil 2",
    deck: {
      "Great Moth": 1,
      Gokibore: 2,
      "Flying Kamakiri #2": 2,
      "Cocoon of Evolution": 3,
      "Kuwagata A": 2,
      "Flying Kamakiri #1": 2,
      "Killer Needle": 2,
      "Parasite Paracide": 2,
      "Perfectly Ultimate Great Moth": 1,
      "Kwagar Hercules": 2,
      "Pinch Hopper": 2,
      "Big Insect": 2,
      "Giant Flea": 2,
      "Hunter Spider": 2,
      "Insect Queen": 1,
      "Laser Cannon Armor": 2,
      "Insect Armor with Laser Cannon": 2,
      Forest: 3,
      "Eradicating Aerosol": 2,
      "Invisible Wire": 1,
      "Infinite Dismissal": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Weevil 3",
    deck: {
      "Larvae Moth": 2,
      "Cocoon of Evolution": 2,
      "Great Moth": 2,
      "Perfectly Ultimate Great Moth": 2,
      "Pinch Hopper": 2,
      "Parasite Paracide": 2,
      "Skull-Mark Lady Bug": 2,
      "Hercules Beetle": 2,
      "Flying Kamakiri #1": 2,
      "Giant Flea": 2,
      "Big Insect": 2,
      Gokibore: 2,
      "Killer Needle": 2,
      "Swords of Revealing Light": 3,
      "Laser Cannon Armor": 3,
      "Eradicating Aerosol": 2,
      "Insect Armor with Laser Cannon": 3,
      "Infinite Dismissal": 3,
    },
    lp: 8000,
    field: "Forest",
    location: "Galapagos",
    payout: 0,
    ante: [
      "Infinite Dismissal",
      "Eradicating Aerosol",
      "Parasite Paracide",
      "Forest",
      "Kwagar Hercules",
      "Hercules Beetle",
      "Laser Cannon Armor",
      "Insect Armor with Laser Cannon",
      "Pinch Hopper",
      "Flying Kamakiri #1",
      "Giant Flea",
      "Flying Kamakiri #2",
      "Great Moth",
      "Perfectly Ultimate Great Moth",
      "Cocoon of Evolution",
      "Javelin Beetle",
    ],
  },
  {
    name: "Weevil (Imposter)",
    deck: {
      "Flying Kamakiri #1": 3,
      "Kwagar Hercules": 3,
      "Flying Kamakiri #2": 3,
      "Kuwagata A": 3,
      "Mystic Tomato": 3,
      "Rose Spectre of Dunn": 3,
      "Giant Scorpion of the Tundra": 3,
      "Bean Soldier": 3,
      "Insect Soldiers of the Sky": 2,
      "Drill Bug": 3,
      "Vile Germs": 2,
      "Darkworld Thorns": 3,
      Forest: 1,
      "Swords of Revealing Light": 1,
      "Goblin's Secret Remedy": 1,
      Ookazi: 1,
      "Acid Trap Hole": 2,
    },
    lp: 10000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Kwagar Hercules",
      "Rose Spectre of Dunn",
      "Insect Soldiers of the Sky",
      "Giant Scorpion of the Tundra",
      "Drill Bug",
      "Darkworld Thorns",
      "Kuwagata A",
      "Flying Kamakiri #1",
      "Mystic Tomato",
      "Bean Soldier",
      "Flying Kamakiri #2",
    ],
  },
  {
    name: "Mako 1",
    deck: {
      "Sea Kamen": 3,
      "Ice Water": 3,
      "Flying Fish": 3,
      "Beastking of the Swamps": 3,
      "Fiend Kraken": 3,
      Jellyfish: 3,
      "Twin Long Rods #1": 3,
      "The Furious Sea King": 3,
      "The Melting Red Shadow": 3,
      "White Dolphin": 3,
      "Kairyu-Shin": 1,
      "Amphibian Beast": 1,
      "Root Water": 3,
      "Fortress Whale": 1,
      "The Legendary Fisherman": 1,
      "Power of Kaishin": 1,
      Umi: 1,
      "Torrential Tribute": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Fiend Kraken",
      "Jellyfish",
      "Kairyu-Shin",
      "Twin Long Rods #1",
      "Flying Fish",
      "Root Water",
      "Amphibian Beast",
      "Beastking of the Swamps",
      "The Furious Sea King",
      "Power of Kaishin",
      "Umi",
      "Ice Water",
      "Sea Kamen",
      "Torrential Tribute",
      "Fortress Whale",
    ],
  },
  {
    name: "Mako 2",
    deck: {
      "Turtle Bird": 1,
      "The Legendary Fisherman": 1,
      "Fiend Kraken": 3,
      "Flying Fish": 3,
      Jellyfish: 3,
      "Beastking of the Swamps": 3,
      Takriminos: 3,
      "Twin Long Rods #1": 3,
      "Wow Warrior": 3,
      "Amphibian Beast": 1,
      "Kairyu-Shin": 2,
      "Fortress Whale": 1,
      "Amazon of the Seas": 3,
      "Spike Seadra": 1,
      "Boulder Tortoise": 1,
      "Power of Kaishin": 3,
      Umi: 3,
      "Bear Trap": 1,
      "Torrential Tribute": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Fiend Kraken",
      "Jellyfish",
      "Kairyu-Shin",
      "Twin Long Rods #1",
      "Flying Fish",
      "Amphibian Beast",
      "Beastking of the Swamps",
      "Power of Kaishin",
      "Umi",
      "Takriminos",
      "Spike Seadra",
      "Boulder Tortoise",
      "Turtle Bird",
      "Wow Warrior",
      "Amazon of the Seas",
      "Bear Trap",
      "Torrential Tribute",
      "Fortress Whale",
    ],
  },
  {
    name: "Mako 3",
    deck: {
      "Giant Red Seasnake": 2,
      "Fortress Whale": 1,
      "Fire Kraken": 2,
      "Aqua Serpent": 3,
      "Great White": 2,
      "Crazy Fish": 2,
      "Amphibian Beast": 3,
      Jellyfish: 1,
      "Fiend Kraken": 2,
      "High Tide Gyojin": 2,
      Takriminos: 2,
      "7 Colored Fish": 2,
      "Kairyu-Shin": 3,
      "Flying Fish": 1,
      "The Legendary Fisherman": 1,
      "Swords of Revealing Light": 2,
      "Power of Kaishin": 3,
      Umi: 2,
      "Acid Trap Hole": 1,
      "Torrential Tribute": 3,
    },
    lp: 8000,
    field: "Umi",
    location: "Galapagos",
    payout: 0,
    ante: [
      "Great White",
      "Kairyu-Shin",
      "Humanoid Slime",
      "Worm Drake",
      "Humanoid Worm Drake",
      "Revival Jam",
      "Amphibian Beast",
      "Aqua Serpent",
      "Power of Kaishin",
      "Umi",
      "Takriminos",
      "Kaiser Sea Horse",
      "7 Colored Fish",
      "Giant Red Seasnake",
      "Crazy Fish",
      "Fire Kraken",
      "High Tide Gyojin",
      "Acid Trap Hole",
      "Torrential Tribute",
      "Fortress Whale",
    ],
  },
  {
    name: "Mako (Imposter)",
    deck: {
      "Crazy Fish": 3,
      "Amazon of the Seas": 3,
      "Water Omotics": 3,
      "Roaring Ocean Snake": 3,
      "Rare Fish": 3,
      "Fire Kraken": 3,
      "Violent Rain": 3,
      "Wow Warrior": 3,
      "High Tide Gyojin": 3,
      "Turtle Bird": 3,
      "Enchanting Mermaid": 2,
      "Goblin's Secret Remedy": 1,
      "Power of Kaishin": 2,
      Umi: 1,
      "Swords of Revealing Light": 1,
      Ookazi: 1,
      "Acid Trap Hole": 2,
    },
    lp: 10000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Roaring Ocean Snake",
      "Rare Fish",
      "Water Omotics",
      "Enchanting Mermaid",
      "Crazy Fish",
      "Fire Kraken",
      "Turtle Bird",
      "Wow Warrior",
      "Violent Rain",
      "High Tide Gyojin",
      "Amazon of the Seas",
    ],
  },
  {
    name: "Mai 1",
    deck: {
      "Harpie Lady": 3,
      Mavelus: 3,
      Peacock: 3,
      "Droll Bird": 3,
      "Ray & Temperature": 3,
      Kurama: 3,
      Niwatori: 3,
      "Harpie Lady Sisters": 1,
      "Princess of Tsurugi": 3,
      "Dragoness the Wicked Knight": 2,
      Lisark: 2,
      "Winged Dragon, Guardian of the Fortress #2": 2,
      "Harpie's Pet Dragon": 1,
      "Elegant Egotist": 1,
      "Follow Wind": 1,
      "Cyber Shield": 1,
      Mountain: 1,
      "Electro-Whip": 1,
      "Harpie's Feather Duster": 1,
      "Amazon Archers": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Mai 2",
    deck: {
      "Sonic Bird": 2,
      "Dragoness the Wicked Knight": 2,
      "Amazon Fighter": 1,
      Peacock: 2,
      Mavelus: 2,
      Takuhee: 2,
      "Kanan the Swordmistress": 2,
      "Winged Dragon, Guardian of the Fortress #2": 2,
      "Harpie Lady Sisters": 1,
      "Amazon Sword Woman": 1,
      "Spirit of the Books": 1,
      "Wing Eagle": 2,
      "Harpie's Pet Dragon": 2,
      Niwatori: 2,
      "Harpie Lady": 3,
      "Fiend Reflection #2": 2,
      "Amazon Chain Master": 1,
      Mountain: 3,
      "Harpie's Feather Duster": 1,
      "Malevolent Nuzzler": 1,
      "Elegant Egotist": 1,
      "Electro-Whip": 1,
      "Cyber Shield": 1,
      "Amazon Archers": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Mai 3",
    deck: {
      "Skull Red Bird": 2,
      "Crimson Sunbird": 2,
      "Harpie's Brother": 2,
      "Harpie Lady Sisters": 2,
      "Cyber Harpie": 2,
      "Amazon Chain Master": 2,
      "Amazon Sword Woman": 2,
      "Harpie Lady": 2,
      "Harpie's Pet Dragon": 2,
      "Amazon Fighter": 2,
      "Spirit of the Books": 3,
      "Swords of Revealing Light": 3,
      "Cyber Shield": 3,
      "Electro-Whip": 3,
      "Elegant Egotist": 3,
      "Follow Wind": 2,
      "Amazon Archers": 3,
    },
    lp: 8000,
    field: "Mountain",
    location: "Domino Pier",
    payout: 0,
    ante: [
      "Elegant Egotist",
      "Harpie's Pet Dragon",
      "Mountain",
      "Amazon Archers",
      "Electro-Whip",
      "Cyber Shield",
      "Follow Wind",
      "Harpie's Feather Duster",
      "Spirit of the Books",
      "Takuhee",
      "Amazon Chain Master",
      "Amazon Fighter",
      "Crimson Sunbird",
      "Skull Red Bird",
      "Amazon Sword Woman",
      "Harpie's Brother",
    ],
  },
  {
    name: "Mai (Imposter)",
    deck: {
      "Skull Red Bird": 3,
      Mavelus: 3,
      "Spirit of the Books": 3,
      "Follow Wind": 2,
      "Senju of the Thousand Hands": 3,
      Skullbird: 3,
      "Fiend Reflection #2": 3,
      "Shining Friendship": 3,
      Takuhee: 3,
      "Faith Bird": 3,
      "Goddess with the Third Eye": 2,
      "Dark Witch": 3,
      "Swords of Revealing Light": 1,
      Ookazi: 1,
      "Goblin's Secret Remedy": 1,
      Mountain: 1,
      "Acid Trap Hole": 2,
    },
    lp: 10000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Dark Witch",
      "Skullbird",
      "Goddess with the Third Eye",
      "Mavelus",
      "Shining Friendship",
      "Spirit of the Books",
      "Senju of the Thousand Hands",
      "Fiend Reflection #2",
      "Takuhee",
      "Faith Bird",
      "Skull Red Bird",
    ],
  },
  {
    name: "Espa 1",
    deck: {
      "Cyber Raider": 3,
      "Oscillo Hero": 3,
      Jinzo: 1,
      "The Last Warrior from Another Planet": 3,
      "UFO Turtle": 3,
      Gradius: 3,
      "Cannon Soldier": 3,
      "The Fiend Megacyber": 1,
      "Ground Attacker Bugroth": 3,
      "Gate Sword": 1,
      "The Inexperienced Spy": 3,
      "Beckon to Darkness": 3,
      "Machine Conversion Factory": 3,
      "Cyclon Laser": 3,
      "Brain Control": 1,
      "Infinite Dismissal": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "UFO Turtle",
      "Cyclon Laser",
      "Ground Attacker Bugroth",
      "Machine Conversion Factory",
      "Gradius",
      "Cannon Soldier",
      "Oscillo Hero",
      "Beckon to Darkness",
      "Infinite Dismissal",
      "Cyber Raider",
      "The Fiend Megacyber",
      "Brain Control",
      "The Inexperienced Spy",
      "Gate Sword",
      "The Last Warrior from Another Planet",
    ],
  },
  {
    name: "Espa 2",
    deck: {
      "Axe Raider": 3,
      "Cyber Raider": 3,
      "The Fiend Megacyber": 2,
      "Cyber Falcon": 3,
      "Ground Attacker Bugroth": 3,
      Mechanicalchaser: 2,
      "Guardian of the Throne Room": 3,
      "Reflect Bounder": 3,
      Gradius: 2,
      "Amphibious Bugroth": 3,
      "Swords of Revealing Light": 3,
      "Machine Conversion Factory": 3,
      "Cyclon Laser": 3,
      "Brain Control": 1,
      "Acid Trap Hole": 3,
    },
    lp: 8000,
    field: "Sogen",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Cyclon Laser",
      "Sogen",
      "Amphibious Bugroth",
      "Acid Trap Hole",
      "Machine Conversion Factory",
      "Gradius",
      "The Fiend Megacyber",
      "Cyber Falcon",
      "Cyber Raider",
      "Ground Attacker Bugroth",
      "Guardian of the Throne Room",
      "Jinzo",
      "Axe Raider",
      "Reflect Bounder",
      "Brain Control",
      "Mechanicalchaser",
    ],
  },
  {
    name: "Espa (Imposter)",
    deck: {
      "Ground Attacker Bugroth": 3,
      "Guardian of the Throne Room": 3,
      "Amphibious Bugroth": 3,
      "Dimensional Warrior": 3,
      Holograh: 2,
      "Oscillo Hero": 3,
      "Stuffed Animal": 3,
      "Gate Sword": 3,
      Ookazi: 1,
      Kojikocy: 3,
      "Fiend Sword": 3,
      "Cannon Soldier": 3,
      "Swords of Revealing Light": 1,
      Sogen: 1,
      "Goblin's Secret Remedy": 1,
      "Legendary Sword": 1,
      "Sword of Dark Destruction": 1,
      "Acid Trap Hole": 2,
    },
    lp: 10000,
    field: "Yami",
    location: "Pegasus Castle",
    payout: 0,
    ante: [
      "Amphibious Bugroth",
      "Gate Sword",
      "Holograh",
      "Dimensional Warrior",
      "Stuffed Animal",
      "Oscillo Hero",
      "Fiend Sword",
      "Cannon Soldier",
      "Kojikocy",
      "Ground Attacker Bugroth",
      "Guardian of the Throne Room",
    ],
  },
  {
    name: "Duel Computer (Novice)",
    deck: {
      "Armed Ninja": 3,
      Ameba: 3,
      "Magician of Faith": 3,
      "Milus Radiant": 2,
      Ooguchi: 2,
      "Pot the Trick": 3,
      "Change Slime": 3,
      "Key Mace": 3,
      Griggle: 3,
      "Queen's Double": 3,
      Bat: 3,
      "Dark Plant": 3,
      "Bone Mouse": 3,
      "Sinister Serpent": 2,
      "The Unhappy Maiden": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "KaibaCorp",
    payout: 0,
    ante: [
      "Wings of Wicked Flame",
      "Djinn the Watcher of the Wind",
      "The Bewitching Phantom Thief",
      "Phantom Dewan",
      "Winged Cleaver",
      "Kaminarikozou",
      "Meotoko",
      "Serpent Marauder",
      "Red Medicine",
      "Hinotama",
      "Tatsunootoshigo",
      "Saber Slasher",
      "Bolt Escargot",
      "Boo Koo",
      "Misairuzame",
      "Morphing Jar",
      "Bear Trap",
    ],
  },
  {
    name: "Duel Computer (Standard)",
    deck: {
      "Guardian of the Throne Room": 2,
      "Darkfire Soldier #2": 2,
      "The Immortal of Thunder": 2,
      "Alligator's Sword": 2,
      "Battle Ox": 2,
      "Whiptail Crow": 2,
      "Man-Eating Treasure Chest": 2,
      "High Tide Gyojin": 2,
      "Neo the Magic Swordsman": 2,
      "Baron of the Fiend Sword": 2,
      "Cosmo Queen": 1,
      "Judge Man": 1,
      "The Earl of Demise": 1,
      "Mr. Volcano": 1,
      Sengenjin: 1,
      "Sea King Dragon": 1,
      "Garma Sword": 1,
      "Rabid Horseman": 1,
      "Kaminari Attack": 1,
      "Rose Spectre of Dunn": 1,
      "Millennium Golem": 1,
      "Tri-Horned Dragon": 1,
      "Punished Eagle": 1,
      "Talons of Shurilane": 1,
      "Soul of the Pure": 2,
      Ookazi: 2,
      "Acid Trap Hole": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "KaibaCorp",
    payout: 0,
    ante: [
      "Battle Ox",
      "Judge Man",
      "Rabid Horseman",
      "Baron of the Fiend Sword",
      "Mr. Volcano",
      "Darkfire Soldier #2",
      "Soul of the Pure",
      "Ookazi",
      "Sea King Dragon",
      "Millennium Golem",
      "Kaminari Attack",
      "The Immortal of Thunder",
      "Punished Eagle",
      "The Earl of Demise",
      "Guardian of the Throne Room",
      "Rose Spectre of Dunn",
      "High Tide Gyojin",
      "Whiptail Crow",
      "Acid Trap Hole",
      "Sengenjin",
      "Tri-Horned Dragon",
      "Cosmo Queen",
      "Garma Sword",
      "Alligator's Sword",
      "Neo the Magic Swordsman",
      "Man-Eating Treasure Chest",
      "Talons of Shurilane",
    ],
  },
  {
    name: "Duel Computer (Expert)",
    deck: {
      "Castle of Dark Illusions": 3,
      "Gemini Elf": 3,
      "Vorse Raider": 3,
      "Dark Elf": 3,
      "Flash Assailant": 3,
      "Blue-Eyes Ultimate Dragon": 1,
      "Blue-Eyes White Dragon": 3,
      "Reflect Bounder": 1,
      "Ancient Lamp": 1,
      "Barrel Dragon": 1,
      "Swords of Revealing Light": 1,
      "Change of Heart": 1,
      "Brain Control": 1,
      "Pot of Greed": 1,
      "Shadow Spell": 2,
      "Beckon to Darkness": 1,
      Megamorph: 1,
      "The Inexperienced Spy": 1,
      "Harpie's Feather Duster": 1,
      "Final Destiny": 1,
      "Crush Card": 1,
      "Torrential Tribute": 1,
      "Widespread Ruin": 2,
      "Amazon Archers": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "KaibaCorp",
    payout: 0,
    ante: [
      "Castle of Dark Illusions",
      "Flash Assailant",
      "Final Destiny",
      "Vorse Raider",
      "Swords of Revealing Light",
      "Super War-Lion",
      "Yamadron",
      "Zera The Mant",
      "Millennium Shield",
      "Fiend's Mirror",
      "Gemini Elf",
      "Ancient Lamp",
      "Dark Elf",
      "Amazon Archers",
      "Megamorph",
      "Crush Card",
      "Shadow Spell",
      "Harpie's Feather Duster",
      "Widespread Ruin",
      "Torrential Tribute",
      "Beckon to Darkness",
      "Performance of Sword",
      "Hungry Burger",
      "Sengenjin",
      "Skull Guardian",
      "Tri-Horned Dragon",
      "Serpent Night Dragon",
      "Cosmo Queen",
      "Chakra",
      "Crab Turtle",
      "Psycho-Puppet",
      "Garma Sword",
      "Javelin Beetle",
      "Dokurorider",
      "Mask of Shine & Dark",
      "Brain Control",
      "Change of Heart",
      "Pot of Greed",
      "The Inexperienced Spy",
    ],
  },
  {
    name: "Maximillion Pegasus (HoE)",
    deck: {
      "Thousand-Eyes Restrict": 3,
      "Castle of Dark Illusions": 3,
      "Big Shield Gardna": 3,
      "Toon Summoned Skull": 3,
      "Blue-Eyes Toon Dragon": 3,
      Relinquished: 3,
      Megamorph: 3,
      "Change of Heart": 3,
      "Harpie's Feather Duster": 3,
      "Monster Reborn": 3,
      "Pot of Greed": 3,
      "Swords of Revealing Light": 3,
      "Torrential Tribute": 3,
    },
    lp: 30000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [
      "Boar Soldier",
      "Vorse Raider",
      "Darkfire Soldier #1",
      "Dragon Capture Jar",
      "Raigeki",
      "Jirai Gumo",
      "Harpie's Brother",
      "Giant Red Seasnake",
      "The Immortal of Thunder",
      "Zombyra the Dark",
      "Gemini Elf",
      "Monster Reborn",
      "Harpie's Feather Duster",
      "Torrential Tribute",
      "Relinquished",
      "Thousand-Eyes Restrict",
      "Man-Eating Treasure Chest",
      "Brain Control",
      "Change of Heart",
    ],
  },
  {
    name: "Paradox (HoE)",
    deck: {
      "Flash Assailant": 3,
      "Boar Soldier": 3,
      "Swords of Revealing Light": 3,
      "Zera The Mant": 1,
      "Black Luster Soldier": 1,
      "Machine King": 1,
      "Aqua Dragon": 1,
      "Gemini Elf": 3,
      "Dark Elf": 3,
      "Monster Reborn": 3,
      Megamorph: 3,
      "Harpie's Feather Duster": 3,
      "Torrential Tribute": 3,
      "Tri-Horned Dragon": 1,
      "Anti Raigeki": 1,
      "Change of Heart": 3,
      "Pot of Greed": 3,
      "Cyber-Tech Alligator": 1,
    },
    lp: 30000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [
      "Mountain",
      "Raigeki",
      "Swords of Revealing Light",
      "Harpie's Brother",
      "Crimson Sunbird",
      "Thunder Nyan Nyan",
      "Monster Reborn",
      "Megamorph",
      "Des Volstgalph",
      "Cyber Harpie",
      "Torrential Tribute",
      "Tri-Horned Dragon",
      "Meteor B. Dragon",
      "Change of Heart",
      "Pot of Greed",
    ],
  },
  {
    name: "Seto Kaiba (HoE)",
    deck: {
      "Blue-Eyes White Dragon": 3,
      "X-Head Cannon": 3,
      "Des Feral Imp": 2,
      "Heavy Storm": 1,
      "Obelisk the Tormentor": 1,
      "Vorse Raider": 3,
      "Dark Energy": 3,
      "Swords of Revealing Light": 2,
      Swordstalker: 1,
      "Blue-Eyes Ultimate Dragon": 1,
      "Ancient Lamp": 1,
      "Blade Knight": 3,
      "Y-Dragon Head": 3,
      "Z-Metal Tank": 3,
      Megamorph: 3,
      "Crush Card": 2,
      "Shadow Spell": 2,
      "Steel Ogre Grotto #2": 1,
      "Invitation to a Dark Sleep": 1,
      "Lord of D.": 1,
    },
    lp: 60000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [
      "Blue-Eyes White Dragon",
      "X-Head Cannon",
      "Flash Assailant",
      "Obelisk the Tormentor",
      "Vorse Raider",
      "Raigeki",
      "Blue-Eyes Ultimate Dragon",
      "Ancient Lamp",
      "Y-Dragon Head",
      "Z-Metal Tank",
      "Monster Reborn",
      "Megamorph",
      "Crush Card",
      "Shadow Spell",
      "Torrential Tribute",
      "Master of Dragon Soldier",
      "Lord of D.",
      "Change of Heart",
    ],
  },
  {
    name: "Dark Joey (HoE)",
    deck: {
      "Gearfried the Iron Knight": 3,
      "Steel Fan Fighter": 1,
      "Boar Soldier": 3,
      "Panther Warrior": 3,
      "Exarion Universe": 1,
      Jinzo: 1,
      "Red-Eyes B. Dragon": 1,
      "Red-Eyes Black Metal Dragon": 1,
      "B. Skull Dragon": 1,
      "Zombyra the Dark": 3,
      "Gilford the Lightning": 1,
      Megamorph: 3,
      "Change of Heart": 3,
      "Harpie's Feather Duster": 3,
      "Monster Reborn": 3,
      "Pot of Greed": 3,
      "Swords of Revealing Light": 3,
      "Torrential Tribute": 3,
    },
    lp: 30000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [
      "Red-Eyes B. Dragon",
      "Gearfried the Iron Knight",
      "Boar Soldier",
      "B. Skull Dragon",
      "Sogen",
      "Raigeki",
      "Tremendous Fire",
      "Zombyra the Dark",
      "Megamorph",
      "Gilford the Lightning",
      "Exarion Universe",
      "Torrential Tribute",
      "Red-Eyes Black Metal Dragon",
      "Panther Warrior",
      "Jinzo",
      "Brain Control",
      "Change of Heart",
      "Steel Fan Fighter",
    ],
  },
  {
    name: "Yami Bakura (HoE)",
    deck: {
      "Headless Knight": 3,
      "The Earl of Demise": 1,
      "Nuvia the Wicked": 3,
      "The Portrait's Secret": 1,
      "The Gross Ghost of Fled Dreams": 1,
      "Flash Assailant": 3,
      "Dark Elf": 3,
      "Earthbound Spirit": 1,
      "Dark Necrofear": 1,
      Gernia: 1,
      Megamorph: 3,
      "Change of Heart": 3,
      "Harpie's Feather Duster": 3,
      "Monster Reborn": 3,
      "Pot of Greed": 3,
      "Swords of Revealing Light": 3,
      "Torrential Tribute": 3,
      "Destiny Board": 1,
    },
    lp: 60000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [
      "Castle of Dark Illusions",
      "King of Yamimakai",
      "Headless Knight",
      "Dark Necrofear",
      "Flash Assailant",
      "Raigeki",
      "Zera The Mant",
      "The Earl of Demise",
      "Earthbound Spirit",
      "Gernia",
      "Destiny Board",
      'Spirit Message "I"',
      'Spirit Message "N"',
      'Spirit Message "A"',
      'Spirit Message "L"',
      "Monster Reborn",
      "Puppet Master",
      "Megamorph",
      "Shadow Spell",
      "Nuvia the Wicked",
      "Torrential Tribute",
      "The Portrait's Secret",
      "The Gross Ghost of Fled Dreams",
      "Change of Heart",
      "Pot of Greed",
    ],
  },
  {
    name: "Yami Yugi (HoE)",
    deck: {
      "Queen's Knight": 3,
      "Jack's Knight": 1,
      "King's Knight": 3,
      "Big Shield Gardna": 3,
      "Dark Elf": 3,
      "Dark Magician": 1,
      "Dark Magician Knight": 1,
      "Dark Magician Girl": 1,
      "Mirage Knight": 1,
      "Dark Flare Knight": 1,
      "Dark Paladin": 1,
      "Flash Assailant": 3,
      Megamorph: 3,
      "Change of Heart": 3,
      "Harpie's Feather Duster": 3,
      "Monster Reborn": 3,
      "Pot of Greed": 3,
      "Swords of Revealing Light": 3,
      "Torrential Tribute": 3,
    },
    lp: 60000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [
      "Mystical Elf",
      "Summoned Skull",
      "Dark Magician",
      "Kuriboh",
      "Swords of Revealing Light",
      "Spellbinding Circle",
      "Queen's Knight",
      "Jack's Knight",
      "King's Knight",
      "Dark Magician Knight",
      "Monster Reborn",
      "Dark Paladin",
      "Dark Flare Knight",
      "Mirage Knight",
      "Harpie's Feather Duster",
      "Torrential Tribute",
      "Big Shield Gardna",
      "Dark Magician Girl",
      "Beast of Gilfer",
      "Brain Control",
      "Change of Heart",
      "Multiply",
      "Pot of Greed",
    ],
  },
  {
    name: "Yami Marik (HoE)",
    deck: {
      "Executor - Makyura": 3,
      "Viser Des": 3,
      "Legendary Fiend": 1,
      "Gil Garth": 3,
      "Lava Golem": 1,
      "Dark Jeroid": 3,
      "The Winged Dragon of Ra (Phoenix Mode)": 1,
      Newdoria: 3,
      "Byser Shock": 1,
      Helpoemer: 1,
      Megamorph: 3,
      "Change of Heart": 3,
      "Harpie's Feather Duster": 3,
      "Monster Reborn": 3,
      "Pot of Greed": 3,
      "Swords of Revealing Light": 3,
      "Torrential Tribute": 3,
    },
    lp: 60000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [
      "Castle of Dark Illusions",
      "Dian Keto the Cure Master",
      "Swords of Revealing Light",
      "The Winged Dragon of Ra (Phoenix Mode)",
      "Gil Garth",
      "Monster Reborn",
      "Helpoemer",
      "Lava Golem",
      "Newdoria",
      "Dark Jeroid",
      "Executor - Makyura",
      "Byser Shock",
      "Viser Des",
      "Harpie's Feather Duster",
      "Legendary Fiend",
      "Torrential Tribute",
      "Brain Control",
      "Change of Heart",
    ],
  },
  {
    name: "Noah (HoE)",
    deck: {
      "Chiron the Mage": 3,
      "Giant Rex": 3,
      "Asura Priest": 3,
      "F.G.D.": 1,
      "Nightmare Penguin": 1,
      Aeris: 3,
      "Berserk Dragon": 1,
      "Yamata Dragon": 1,
      "Perfect Machine King": 1,
      "Shinato, King of a Higher Plane": 1,
      Megamorph: 3,
      "Change of Heart": 3,
      "Harpie's Feather Duster": 3,
      "Monster Reborn": 3,
      "Pot of Greed": 3,
      "Swords of Revealing Light": 3,
      "Torrential Tribute": 3,
      "Anti Raigeki": 1,
    },
    lp: 60000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [],
  },
  {
    name: "Shadi (HoE)",
    deck: {
      "Dark Elf": 3,
      Sengenjin: 1,
      "Flash Assailant": 3,
      "Gemini Elf": 3,
      "Cosmo Queen": 3,
      "Magician of Black Chaos": 3,
      "Boar Soldier": 3,
      "Bracchio-raidus": 1,
      "Firewing Pegasus": 1,
      "Crimson Sunbird": 1,
      Megamorph: 3,
      "Change of Heart": 3,
      "Harpie's Feather Duster": 3,
      "Monster Reborn": 3,
      "Pot of Greed": 3,
      "Swords of Revealing Light": 3,
      "Torrential Tribute": 3,
      "Anti Raigeki": 1,
    },
    lp: 30000,
    field: "Yami",
    location: "Hall of Eternity",
    payout: 0,
    ante: [],
  },
  {
    name: "guy with the red hat in domino square (Opponent 1)",
    deck: {
      "Ancient Jar": 3,
      "Shadow Specter": 3,
      "Change Slime": 3,
      "Dark Plant": 3,
      Haniwa: 3,
      "Key Mace": 3,
      "Skull Servant": 3,
      "Steel Scorpion": 3,
      Tomozaurus: 3,
      "Mystic Lamp": 3,
      "Zone Eater": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Shadow Specter",
      "Skull Servant",
      "Tomozaurus",
      "Dark Plant",
      "Ancient Jar",
      "Key Mace",
      "Haniwa",
      "Change Slime",
      "Mystic Lamp",
      "Zone Eater",
      "Steel Scorpion",
      "Gravedigger Ghoul",
      "Fake Trap",
      "The Inexperienced Spy",
    ],
  },
  {
    name: "other guy in domino square",
    deck: {
      "Dancing Elf": 3,
      Leghul: 3,
      "Jinzo #7": 3,
      Ooguchi: 3,
      "Monster Eye": 3,
      "Armed Ninja": 3,
      "White Dolphin": 3,
      "Turu-Purun": 3,
      "Mechanical Spider": 3,
      Bat: 3,
      "Magician of Faith": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Guy above Domino Square",
    deck: {
      "Zone Eater": 3,
      "Rainbow Flower": 3,
      "Queen's Double": 3,
      "Magician of Faith": 3,
      Haniwa: 3,
      "Mechanical Spider": 3,
      Griggle: 3,
      Leghul: 3,
      "Shadow Specter": 3,
      "Armed Ninja": 3,
      "Dark Plant": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "guy right of domino square with glasses",
    deck: {
      "Dark Plant": 3,
      "Change Slime": 3,
      "Mechanical Spider": 3,
      "Turu-Purun": 3,
      Ameba: 3,
      "Zone Eater": 3,
      Griggle: 3,
      "Dancing Elf": 3,
      Tomozaurus: 3,
      "Sinister Serpent": 3,
      "Key Mace": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "other guy right of domino square",
    deck: {
      Griggle: 3,
      "Queen's Double": 3,
      "Pot the Trick": 3,
      "Shadow Specter": 3,
      Ameba: 3,
      "Rainbow Flower": 3,
      "Milus Radiant": 3,
      "Bone Mouse": 3,
      "Sinister Serpent": 3,
      "Hane-Hane": 3,
      "Bite Shoes": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "guy above that area",
    deck: {
      "Bite Shoes": 3,
      "Zone Eater": 3,
      Leghul: 3,
      Griggle: 3,
      "Shadow Specter": 3,
      "Magician of Faith": 3,
      "Milus Radiant": 3,
      "Turu-Purun": 3,
      "Key Mace": 3,
      "Ancient Jar": 3,
      "Change Slime": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "guy with red hair under that area",
    deck: {
      "Queen's Double": 3,
      "Steel Scorpion": 3,
      "Jinzo #7": 3,
      "Mystic Lamp": 3,
      "Skull Servant": 3,
      "Bone Mouse": 3,
      "Ancient Jar": 3,
      "Armed Ninja": 3,
      Ameba: 3,
      "Bite Shoes": 3,
      "Magician of Faith": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "other guy",
    deck: {
      Bat: 3,
      "Mystic Lamp": 3,
      "Dancing Elf": 3,
      Haniwa: 3,
      "Turu-Purun": 3,
      Griggle: 3,
      "Monster Eye": 3,
      Ameba: 3,
      "Ancient Jar": 3,
      "Zone Eater": 3,
      "White Dolphin": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "guy in area south of clocktower square",
    deck: {
      "Sinister Serpent": 3,
      "Change Slime": 3,
      "Dark Plant": 3,
      "Queen's Double": 3,
      "Hane-Hane": 3,
      "Key Mace": 3,
      Leghul: 3,
      "Milus Radiant": 3,
      "Armed Ninja": 3,
      "Steel Scorpion": 3,
      "Bone Mouse": 3,
      "Fake Trap": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "guy with yellow hair left of that area",
    deck: {
      "Shadow Specter": 3,
      "Rainbow Flower": 3,
      "Magician of Faith": 3,
      "Pot the Trick": 3,
      "Mechanical Spider": 3,
      "Skull Servant": 3,
      Hyozanryu: 3,
      Ooguchi: 3,
      "Bite Shoes": 3,
      Tomozaurus: 3,
      "Jinzo #7": 3,
      "Gravedigger Ghoul": 3,
      "The Inexperienced Spy": 1,
      "Fake Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Conductor (Turnstyles)",
    deck: {
      "Morphing Jar #2": 1,
      "Jinzo #7": 1,
      "Magician of Faith": 1,
      Kaminarikozou: 1,
      "The Bewitching Phantom Thief": 1,
      "The Furious Sea King": 1,
      "Lightning Conger": 1,
      "Wicked Mirror": 1,
      "Weather Control": 1,
      "Mechanical Spider": 1,
      "Dancing Elf": 1,
      "Droll Bird": 1,
      "Curtain of the Dark Ones": 1,
      "Star Boy": 1,
      "Doma The Angel of Silence": 1,
      Tatsunootoshigo: 1,
      Tomozaurus: 1,
      "Flame Dancer": 1,
      "Cyber Commander": 1,
      "Candle of Fate": 1,
      Genin: 1,
      "Mystic Lamp": 1,
      Leogun: 1,
      "Winged Cleaver": 1,
      "Ancient Jar": 1,
      "LaLa Li-oon": 1,
      "Hinotama Soul": 1,
      "Petit Dragon": 1,
      "Armed Ninja": 1,
      "Hane-Hane": 1,
      Peacock: 1,
      "Phantom Dewan": 1,
      "Boo Koo": 1,
      "Monster Eye": 1,
      "Beast of Talwar": 1,
      "Beckon to Darkness": 1,
      "Heavy Storm": 1,
      "Dark-Piercing Light": 1,
      "Restructer Revolution": 1,
      Eatgaboon: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Domino Station",
    payout: 200,
    ante: [
      "Dark-Piercing Light",
      "Beast of Talwar",
      "Doma The Angel of Silence",
      "Tatsunootoshigo",
      "Beckon to Darkness",
      "Dancing Elf",
      "Armed Ninja",
      "Ancient Jar",
      "Magician of Faith",
      "Peacock",
      "Leogun",
      "Flame Dancer",
      "Star Boy",
      "Curtain of the Dark Ones",
      "Hinotama Soul",
      "Winged Cleaver",
      "Kaminarikozou",
      "Cyber Commander",
      "Lightning Conger",
      "The Furious Sea King",
      "Morphing Jar #2",
      "Genin",
      "Restructer Revolution",
      "Heavy Storm",
    ],
  },
  {
    name: "guy in same area of Conductor (Turnstyles)",
    deck: {
      "Hiro's Shadow Scout": 3,
      "The Melting Red Shadow": 3,
      "Jinzo #7": 3,
      "Cyber-Stein": 3,
      "Air Marmot of Nefariousness": 3,
      "Phantom Dewan": 3,
      Meotoko: 3,
      "Pot the Trick": 3,
      "Mystic Lamp": 3,
      "Curtain of the Dark Ones": 3,
      "Shadow Specter": 3,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "guy with brown (long) hair at train station (Opponent 19)",
    deck: {
      "Morphing Jar": 3,
      "The Bewitching Phantom Thief": 3,
      "Muka Muka": 3,
      "Key Mace": 3,
      "Weather Control": 3,
      "Boo Koo": 3,
      "Wicked Mirror": 3,
      Haniwa: 3,
      Tomozaurus: 3,
      "Magician of Faith": 3,
      "Ancient Jar": 3,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Tomozaurus",
      "Ancient Jar",
      "Winged Cleaver",
      "Haniwa",
      "Mechanical Spider",
      "Korogashi",
      "Man-Eater Bug",
      "Gale Dogra",
      "Muka Muka",
      "Tentacle Plant",
      "Morphing Jar",
      "House of Adhesive Tape",
      "Darkness Approaches",
      "Anti Raigeki",
    ],
  },
  {
    name: "guy with black, spikey hair at train station (Opponent 20)",
    deck: {
      Tomozaurus: 3,
      Korogashi: 3,
      "Ancient Jar": 3,
      "Morphing Jar": 3,
      "Tentacle Plant": 3,
      "Winged Cleaver": 3,
      Haniwa: 3,
      "Gale Dogra": 3,
      "Man-Eater Bug": 3,
      "Muka Muka": 3,
      "Mechanical Spider": 3,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Tomozaurus",
      "Ancient Jar",
      "Winged Cleaver",
      "Haniwa",
      "Mechanical Spider",
      "Korogashi",
      "Man-Eater Bug",
      "Gale Dogra",
      "Muka Muka",
      "Tentacle Plant",
      "Morphing Jar",
      "House of Adhesive Tape",
      "Darkness Approaches",
      "Anti Raigeki",
    ],
  },
  {
    name: "guy with black hair in a ponytail at train station (opponent 18)",
    deck: {
      Kaminarikozou: 3,
      "Candle of Fate": 3,
      Firegrass: 3,
      "Fire Reaper": 3,
      "Petit Dragon": 3,
      "Little Chimera": 3,
      "LaLa Li-oon": 3,
      "Droll Bird": 3,
      "Lightning Conger": 3,
      "Hane-Hane": 3,
      "Hinotama Soul": 3,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Fire Reaper",
      "Firegrass",
      "LaLa Li-oon",
      "Petit Dragon",
      "Droll Bird",
      "Hinotama Soul",
      "Kaminarikozou",
      "Candle of Fate",
      "Hane-Hane",
      "Little Chimera",
      "Lightning Conger",
      "House of Adhesive Tape",
      "Darkness Approaches",
      "Anti Raigeki",
    ],
  },
  {
    name: "the last guy at train station (Opponent 17)",
    deck: {
      "Tentacle Plant": 3,
      "Man-Eater Bug": 3,
      "White Dolphin": 3,
      Zarigun: 3,
      "Winged Cleaver": 3,
      "Frog the Jam": 3,
      "Turu-Purun": 3,
      "Gale Dogra": 3,
      Korogashi: 3,
      "Serpent Marauder": 3,
      "Star Boy": 3,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Winged Cleaver",
      "Serpent Marauder",
      "White Dolphin",
      "Turu-Purun",
      "Zarigun",
      "Korogashi",
      "Man-Eater Bug",
      "Gale Dogra",
      "Star Boy",
      "Frog the Jam",
      "Tentacle Plant",
      "House of Adhesive Tape",
      "Darkness Approaches",
      "Anti Raigeki",
    ],
  },
  {
    name: "Conductor (Platform)",
    deck: {
      "Morphing Jar": 3,
      "Ancient Jar": 3,
      Tomozaurus: 3,
      "Star Boy": 3,
      "Frog the Jam": 3,
      "Muka Muka": 3,
      "Turu-Purun": 3,
      Zarigun: 3,
      "White Dolphin": 3,
      Haniwa: 3,
      "Serpent Marauder": 3,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Tomozaurus",
      "Ancient Jar",
      "Haniwa",
      "Serpent Marauder",
      "White Dolphin",
      "Turu-Purun",
      "Zarigun",
      "Muka Muka",
      "Star Boy",
      "Frog the Jam",
      "Morphing Jar",
      "House of Adhesive Tape",
      "Darkness Approaches",
      "Anti Raigeki",
    ],
  },
  {
    name: "guy in white shirt in train",
    deck: {
      "Morphing Jar": 3,
      "Boo Koo": 3,
      "Candle of Fate": 3,
      "The Bewitching Phantom Thief": 3,
      "Mystic Lamp": 3,
      "Hane-Hane": 3,
      Korogashi: 3,
      "The Melting Red Shadow": 3,
      "Frog the Jam": 3,
      "Lightning Conger": 3,
      "Kaiser Dragon": 1,
      "Meteor Dragon": 1,
      "Meteor B. Dragon": 1,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "other guy in train (Opponent 23)",
    deck: {
      "Petit Dragon": 3,
      "Little Chimera": 3,
      "Droll Bird": 3,
      LaMoon: 1,
      "Weather Control": 3,
      Kaminarikozou: 3,
      "The Bewitching Phantom Thief": 3,
      "Key Mace": 3,
      "Hane-Hane": 3,
      "Wicked Mirror": 3,
      "LaLa Li-oon": 3,
      "Bolt Escargot": 1,
      Ocubeam: 1,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "LaMoon",
      "Weather Control",
      "The Bewitching Phantom Thief",
      "LaLa Li-oon",
      "Key Mace",
      "Petit Dragon",
      "Droll Bird",
      "Kaminarikozou",
      "Wicked Mirror",
      "Ocubeam",
      "Bolt Escargot",
      "Hane-Hane",
      "Little Chimera",
    ],
  },
  {
    name: "Conductor (Carriage)",
    deck: {
      LaMoon: 1,
      Genin: 3,
      Kanikabuto: 3,
      Kiseitai: 3,
      "Turtle Raccoon": 3,
      "Monster Egg": 3,
      Ganigumo: 3,
      "Dig Beak": 3,
      "Djinn the Watcher of the Wind": 3,
      "Stone D.": 1,
      "Petit Angel": 3,
      "Lucky Trinket": 3,
      "Darkness Approaches": 3,
      "Saber Slasher": 1,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Duel Express",
    payout: 96,
    ante: [
      "LaMoon",
      "Djinn the Watcher of the Wind",
      "Monster Egg",
      "Dig Beak",
      "Lucky Trinket",
      "Genin",
      "Petit Angel",
      "Kiseitai",
      "Saber Slasher",
      "Stone D.",
      "Kanikabuto",
      "Ganigumo",
      "Turtle Raccoon",
    ],
  },
  {
    name: "girl in suite back in train",
    deck: {
      "Droll Bird": 3,
      "White Dolphin": 3,
      "Turu-Purun": 3,
      "Hourglass of Life": 1,
      "Weather Control": 3,
      "Petit Dragon": 3,
      "Boo Koo": 3,
      "Time Wizard": 3,
      "Magician of Faith": 3,
      Haniwa: 3,
      Kaminarikozou: 3,
      "Air Marmot of Nefariousness": 3,
      "Key Mace": 3,
      "Phantom Dewan": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "father of the girl",
    deck: {
      "Ryu-Kishin": 3,
      "The Bewitching Phantom Thief": 3,
      "Pot the Trick": 3,
      "Morphing Jar #2": 3,
      "Silver Bow and Arrow": 1,
      "White Magical Hat": 3,
      "Cyber Jar": 3,
      "Blue-Eyed Silver Zombie": 3,
      Prisman: 3,
      "Blast Juggler": 3,
      "Bite Shoes": 3,
      Mikazukinoyaiba: 1,
      Leogun: 1,
      Ocubeam: 1,
      "Pot of Greed": 1,
      "Horn of Light": 1,
      "Black Pendant": 1,
      "Violet Crystal": 1,
      "Dragon Treasure": 1,
      Eatgaboon: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Kaiba impersonator 1",
    deck: {
      "White Dolphin": 3,
      "Sinister Serpent": 3,
      Tomozaurus: 3,
      "Emperor of the Land and Sea": 1,
      "Petit Dragon": 3,
      "LaLa Li-oon": 3,
      Kaminarikozou: 3,
      "Serpent Marauder": 3,
      "Armed Ninja": 3,
      "Queen's Double": 3,
      "Droll Bird": 3,
      Trent: 1,
      "Spike Seadra": 1,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [
      "Tomozaurus",
      "LaLa Li-oon",
      "Petit Dragon",
      "Droll Bird",
      "Kaminarikozou",
      "Serpent Marauder",
      "Emperor of the Land and Sea",
      "White Dolphin",
      "Spike Seadra",
      "Armed Ninja",
      "Sinister Serpent",
      "Queen's Double",
      "Trent",
    ],
  },
  {
    name: "man at train station",
    deck: {
      "Petit Dragon": 3,
      "Flame Dancer": 3,
      "Turu-Purun": 3,
      "The Melting Red Shadow": 3,
      "Weather Control": 3,
      Tomozaurus: 3,
      "LaLa Li-oon": 3,
      "Gale Dogra": 3,
      "Mystic Lamp": 3,
      "Wicked Mirror": 3,
      "Monster Eye": 3,
      "Darkness Approaches": 3,
      "Anti Raigeki": 3,
      "House of Adhesive Tape": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Takeshi",
    deck: {
      "Morphing Jar": 1,
      "Wicked Mirror": 1,
      "Serpent Marauder": 1,
      "Little Chimera": 1,
      "Star Boy": 1,
      Zarigun: 1,
      "Candle of Fate": 1,
      "Petit Dragon": 1,
      "Fiend's Hand": 1,
      "Frog the Jam": 1,
      "LaLa Li-oon": 1,
      Hoshiningen: 1,
      Kaminarikozou: 1,
      "Witch's Apprentice": 1,
      "Tentacle Plant": 1,
      "Curtain of the Dark Ones": 1,
      "Cyber-Stein": 1,
      "Boo Koo": 1,
      "The Melting Red Shadow": 1,
      "Phantom Dewan": 1,
      "Hourglass of Life": 1,
      Meotoko: 1,
      "Wings of Wicked Flame": 1,
      "Gale Dogra": 1,
      "Flame Dancer": 1,
      Firegrass: 1,
      "The Bewitching Phantom Thief": 1,
      "Fire Reaper": 1,
      "Basic Insect": 1,
      "Man-Eater Bug": 1,
      "Hiro's Shadow Scout": 1,
      "Winged Cleaver": 1,
      Haniwa: 1,
      "Droll Bird": 1,
      "Hinotama Soul": 1,
      "Turu-Purun": 1,
      "Hane-Hane": 1,
      "The Inexperienced Spy": 1,
      "Pot of Greed": 1,
      Eatgaboon: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Domino Pier",
    payout: 0,
    ante: [
      "Basic Insect",
      "Wings of Wicked Flame",
      "Curtain of the Dark Ones",
      "Fiend's Hand",
      "The Bewitching Phantom Thief",
      "The Melting Red Shadow",
      "Fire Reaper",
      "Firegrass",
      "Phantom Dewan",
      "LaLa Li-oon",
      "Petit Dragon",
      "Droll Bird",
      "Winged Cleaver",
      "Hinotama Soul",
      "Kaminarikozou",
      "Meotoko",
      "Hourglass of Life",
      "Haniwa",
      "Candle of Fate",
      "Wicked Mirror",
      "Serpent Marauder",
      "Cyber-Stein",
      "Turu-Purun",
      "Zarigun",
      "Boo Koo",
      "Hoshiningen",
      "Man-Eater Bug",
      "Flame Dancer",
      "Gale Dogra",
      "Star Boy",
      "Hane-Hane",
      "Frog the Jam",
      "Tentacle Plant",
      "Morphing Jar",
      "Little Chimera",
      "Hiro's Shadow Scout",
      "Witch's Apprentice",
      "Eatgaboon",
      "Pot of Greed",
      "The Inexperienced Spy",
    ],
  },
  {
    name: "Casino Wife 1",
    deck: {
      "Wicked Mirror": 3,
      "Bite Shoes": 3,
      "Hiro's Shadow Scout": 3,
      "Dancing Elf": 3,
      Kiseitai: 3,
      "Weather Control": 3,
      "The Shadow Who Controls the Dark": 3,
      "Candle of Fate": 3,
      "Happy Lover": 3,
      "Needle Ball": 3,
      "Key Mace": 3,
      "Winged Trumpeter": 2,
      "Malevolent Nuzzler": 2,
      "Exile of the Wicked": 2,
      Eatgaboon: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Domino Pier",
    payout: 0,
    ante: [
      "Weather Control",
      "The Shadow Who Controls the Dark",
      "Key Mace",
      "Happy Lover",
      "Candle of Fate",
      "Wicked Mirror",
      "Kiseitai",
      "Malevolent Nuzzler",
      "Dancing Elf",
      "Needle Ball",
      "Hiro's Shadow Scout",
      "Winged Trumpeter",
      "Eatgaboon",
      "Bite Shoes",
      "Exile of the Wicked",
    ],
  },
  {
    name: "guy in canada (not in the cave) on the far left",
    deck: {
      Dissolverock: 3,
      "Cyber Jar": 3,
      "Morphing Jar #2": 3,
      Prisman: 3,
      "Rock Ogre Grotto #1": 3,
      Haniwa: 3,
      "Stone Ogre Grotto": 1,
      "Barrel Rock": 3,
      "Morphing Jar": 3,
      "Millennium Golem": 1,
      "Mystical Sand": 1,
      "Muka Muka": 3,
      "Rock Ogre Grotto #2": 3,
      "Stone D.": 1,
      Invigoration: 3,
      Eatgaboon: 2,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "other guy in canada outside the cave",
    deck: {
      "Mega Thunderball": 3,
      "Electric Lizard": 3,
      "Ray & Temperature": 3,
      "Nightmare Scorpion": 3,
      "LaLa Li-oon": 3,
      "Petit Dragon": 3,
      Prisman: 3,
      "Djinn the Watcher of the Wind": 3,
      "Stone Ogre Grotto": 1,
      "Stone D.": 1,
      "Mountain Warrior": 3,
      "Thunder Dragon": 1,
      Ocubeam: 1,
      Invigoration: 3,
      "Gravedigger Ghoul": 3,
      Eatgaboon: 2,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Guy with red hair in area left",
    deck: {
      "Electric Snake": 3,
      Wetha: 3,
      "Kaminari Attack": 3,
      "Oscillo Hero #2": 3,
      "Toad Master": 3,
      "The Furious Sea King": 3,
      "Turtle Bird": 3,
      "Electric Lizard": 3,
      "Water Element": 3,
      Monsturtle: 3,
      "Crab Turtle": 3,
      "Twin Long Rods #1": 3,
      "Power of Kaishin": 1,
      Umi: 1,
      Invigoration: 1,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Other guy",
    deck: {
      "Super War-Lion": 3,
      Fusionist: 3,
      "Dungeon Worm": 2,
      Alinsection: 3,
      "Laughing Flower": 3,
      Snakeyashi: 3,
      "Insect Soldiers of the Sky": 3,
      "Mystical Sheep #2": 3,
      "Man-Eating Plant": 3,
      Trent: 2,
      "Flower Wolf": 2,
      "Nightmare Scorpion": 3,
      Minar: 3,
      "Beast Fangs": 1,
      "Vile Germs": 1,
      Forest: 1,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Guy dressed in blue",
    deck: {
      "Mega Thunderball": 3,
      "Droll Bird": 3,
      Peacock: 2,
      "Oscillo Hero #2": 3,
      "Wicked Dragon with the Ersatz Head": 3,
      "Serpent Marauder": 3,
      "Petit Dragon": 3,
      "Electric Snake": 3,
      Kurama: 3,
      Niwatori: 3,
      "Bolt Escargot": 2,
      "Sky Dragon": 2,
      "Twin-Headed Thunder Dragon": 3,
      "Dragon Treasure": 1,
      "Follow Wind": 1,
      Mountain: 1,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "guy with yellow hair and glasses in area right",
    deck: {
      "Koumori Dragon": 3,
      "The Wicked Worm Beast": 3,
      "Rude Kaiser": 1,
      "Tiger Axe": 3,
      "Zombie Warrior": 3,
      "Ryu-Kishin": 3,
      "Blackland Fire Dragon": 3,
      "Winged Dragon, Guardian of the Fortress #1": 3,
      Garoozis: 1,
      "Battle Steer": 1,
      "The Last Warrior from Another Planet": 1,
      "Mountain Warrior": 3,
      Ansatsu: 1,
      "Sword Hunter": 1,
      "Hitotsu-Me Giant": 3,
      "Zombyra the Dark": 1,
      "Succubus Knight": 1,
      "Axe of Despair": 1,
      Sogen: 1,
      "Sword of Dark Destruction": 1,
      "Legendary Sword": 1,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Area above guy",
    deck: {
      Zoa: 3,
      "Necrolancer the Timelord": 3,
      "Djinn the Watcher of the Wind": 3,
      Sangan: 3,
      "Needle Ball": 3,
      Nemuriko: 3,
      "Flame Manipulator": 3,
      Morinphen: 3,
      "Giant Germ": 3,
      "Mask of Darkness": 3,
      "Ryu-Kishin": 3,
      "Musician King": 3,
      Yami: 1,
      "Black Pendant": 1,
      "Malevolent Nuzzler": 1,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Guy with red hair in area right-under",
    deck: {
      "Princess of Tsurugi": 3,
      "Battle Steer": 3,
      "Zombie Warrior": 3,
      "Lava Battleguard": 3,
      "Mountain Warrior": 3,
      "M-Warrior #1": 3,
      "Monster Egg": 3,
      "Swamp Battleguard": 3,
      "Garnecia Elefantis": 3,
      Doron: 3,
      "Battle Warrior": 3,
      "M-Warrior #2": 3,
      "Sword of Dark Destruction": 1,
      "Legendary Sword": 1,
      Sogen: 1,
      "Bear Trap": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "1 of those guys in Canada",
    deck: {
      "Mystical Sand": 2,
      Prisman: 3,
      Trakadon: 3,
      "Two-Mouth Darkruler": 3,
      "Zombie Warrior": 3,
      Uraby: 3,
      "Great Mammoth of Goldfine": 2,
      "Little D": 3,
      Tomozaurus: 2,
      "The Statue of Easter Island": 3,
      Yaranzo: 3,
      "Bracchio-raidus": 2,
      Invigoration: 1,
      "Mooyan Curry": 2,
      Wasteland: 1,
      "Raise Body Heat": 1,
      "Bear Trap": 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Casino Wife 2",
    deck: {
      Sangan: 3,
      "Invader from Another Dimension": 3,
      "Muse-A": 3,
      "Goddess with the Third Eye": 3,
      "Dark King of the Abyss": 3,
      "Goddess of Whim": 3,
      Skelengel: 3,
      "Winged Trumpeter": 2,
      "Versago the Destroyer": 3,
      "Mask of Darkness": 3,
      "Happy Lover": 3,
      "Ray & Temperature": 3,
      "Malevolent Nuzzler": 2,
      Megamorph: 1,
      "Invisible Wire": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Domino Pier",
    payout: 0,
    ante: [
      "Sangan",
      "Mask of Darkness",
      "Goddess with the Third Eye",
      "Dark King of the Abyss",
      "Happy Lover",
      "Versago the Destroyer",
      "Ray & Temperature",
      "Malevolent Nuzzler",
      "Goddess of Whim",
      "Skelengel",
      "Invader from Another Dimension",
      "Muse-A",
      "Megamorph",
      "Winged Trumpeter",
      "Invisible Wire",
    ],
  },
  {
    name: "Casino Husband",
    deck: {
      "Cosmo Queen": 1,
      "Javelin Beetle": 1,
      "Skull Guardian": 1,
      Chakra: 1,
      "Water Omotics": 1,
      "Hitotsu-Me Giant": 1,
      "Fiend Reflection #1": 1,
      "Fiend's Mirror": 1,
      Yamadron: 1,
      "Performance of Sword": 1,
      "Millennium Shield": 1,
      "Mountain Warrior": 1,
      "Mask of Darkness": 1,
      "Crab Turtle": 1,
      "Super War-Lion": 1,
      "Hungry Burger": 1,
      Dokurorider: 1,
      "Versago the Destroyer": 1,
      "M-Warrior #1": 1,
      "Queen of Autumn Leaves": 1,
      "Succubus Knight": 1,
      "Temple of Skulls": 1,
      "Mask of Shine & Dark": 1,
      "Zera The Mant": 1,
      "Serpent Night Dragon": 1,
      "One-Eyed Shield Dragon": 1,
      "Tri-Horned Dragon": 1,
      "Mysterious Puppeteer": 1,
      "Ryu-Kishin Powered": 1,
      "Psycho-Puppet": 1,
      "Garma Sword": 1,
      Leogun: 1,
      "Darkfire Dragon": 1,
      "Hercules Beetle": 1,
      Sengenjin: 1,
      "30,000-Year White Turtle": 1,
      "B. Dragon Jungle King": 1,
      Megamorph: 1,
      "Invisible Wire": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Domino Pier",
    payout: 0,
    ante: [
      "Hitotsu-Me Giant",
      "Mountain Warrior",
      "Hercules Beetle",
      "Mask of Darkness",
      "Temple of Skulls",
      "M-Warrior #1",
      "Mysterious Puppeteer",
      "Darkfire Dragon",
      "Water Omotics",
      "Versago the Destroyer",
      "One-Eyed Shield Dragon",
      "Super War-Lion",
      "Yamadron",
      "Zera The Mant",
      "Millennium Shield",
      "Fiend's Mirror",
      "Ryu-Kishin Powered",
      "Leogun",
      "30,000-Year White Turtle",
      "B. Dragon Jungle King",
      "Fiend Reflection #1",
      "Succubus Knight",
      "Queen of Autumn Leaves",
      "Megamorph",
      "Invisible Wire",
      "Performance of Sword",
      "Hungry Burger",
      "Sengenjin",
      "Skull Guardian",
      "Tri-Horned Dragon",
      "Serpent Night Dragon",
      "Cosmo Queen",
      "Chakra",
      "Crab Turtle",
      "Psycho-Puppet",
      "Garma Sword",
      "Javelin Beetle",
      "Dokurorider",
      "Mask of Shine & Dark",
    ],
  },
  {
    name: "Kaiba impersonator 2",
    deck: {
      Dissolverock: 3,
      "Giant Germ": 3,
      Doron: 3,
      "Wicked Dragon with the Ersatz Head": 3,
      "Mystical Sheep #2": 3,
      "Kairyu-Shin": 1,
      Prisman: 3,
      "Goddess of Whim": 3,
      "Reaper of the Cards": 1,
      "Insect Soldiers of the Sky": 3,
      "Nimble Momonga": 3,
      Trent: 1,
      "Oscillo Hero #2": 3,
      "Talons of Shurilane": 1,
      "Yamata Dragon": 1,
      "Spirit of the Mountain": 1,
      Megamorph: 2,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Egypt Exhibition",
    payout: 0,
    ante: [
      "Oscillo Hero #2",
      "Kairyu-Shin",
      "Reaper of the Cards",
      "Giant Germ",
      "Nimble Momonga",
      "Doron",
      "Dissolverock",
      "Mystical Sheep #2",
      "Wicked Dragon with the Ersatz Head",
      "Goddess of Whim",
      "Insect Soldiers of the Sky",
      "Prisman",
      "Spirit of the Mountain",
      "Trent",
      "Yamata Dragon",
      "Megamorph",
      "Bear Trap",
      "Talons of Shurilane",
    ],
  },
  {
    name: "Monkey #1",
    deck: {
      "Bolt Penguin": 3,
      "Blast Juggler": 3,
      "Shovel Crusher": 3,
      Lisark: 3,
      "Toad Master": 3,
      "Patrol Robo": 3,
      "Nekogal #1": 3,
      "Cyber Commander": 3,
      "Witch of the Black Forest": 3,
      "Mystical Sheep #1": 3,
      Holograh: 3,
      "Frenzied Panda": 3,
      Raigeki: 1,
      "Dark Hole": 1,
      "Heavy Storm": 1,
      "Final Destiny": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Monkey #2",
    deck: {
      "Bolt Penguin": 3,
      "Blast Juggler": 3,
      "Shovel Crusher": 3,
      Lisark: 3,
      "Toad Master": 3,
      "Patrol Robo": 3,
      "Nekogal #1": 3,
      "Cyber Commander": 3,
      "Witch of the Black Forest": 3,
      "Mystical Sheep #1": 3,
      Holograh: 3,
      "Frenzied Panda": 3,
      Hinotama: 1,
      "Final Flame": 1,
      Ookazi: 1,
      "Tremendous Fire": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Monkey #3",
    deck: {
      "Bolt Penguin": 3,
      "Blast Juggler": 3,
      "Shovel Crusher": 3,
      Lisark: 3,
      "Toad Master": 3,
      "Patrol Robo": 3,
      "Nekogal #1": 3,
      "Cyber Commander": 3,
      "Witch of the Black Forest": 3,
      "Mystical Sheep #1": 3,
      Holograh: 3,
      "Frenzied Panda": 3,
      "Bear Trap": 1,
      "Invisible Wire": 1,
      "Acid Trap Hole": 1,
      "Widespread Ruin": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Monkey #4",
    deck: {
      Griggle: 3,
      "The Unhappy Maiden": 3,
      "Sinister Serpent": 3,
      "Milus Radiant": 3,
      "Armed Ninja": 3,
      "Queen's Double": 3,
      "Bone Mouse": 1,
      "Change Slime": 1,
      "Dancing Elf": 3,
      Ooguchi: 3,
      "Key Mace": 1,
      "Steel Scorpion": 3,
      Ameba: 3,
      Bat: 3,
      "Zone Eater": 3,
      "Ancient Jar": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Ghoul left of Rex",
    deck: {
      "Horn Imp": 3,
      Solitude: 3,
      "Saber Slasher": 1,
      "Stuffed Animal": 3,
      "Giant Germ": 3,
      "Clown Zombie": 3,
      "Mystic Clown": 3,
      "Claw Reacher": 3,
      Zoa: 1,
      "Succubus Knight": 1,
      "Versago the Destroyer": 3,
      "Fiend Sword": 3,
      "Dark King of the Abyss": 3,
      Hinotama: 2,
      "Red Medicine": 2,
      "Black Pendant": 1,
      Eatgaboon: 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Canada",
    payout: 390,
    ante: [
      "Saber Slasher",
      "Succubus Knight",
      "Zoa",
      "Claw Reacher",
      "Giant Germ",
      "Solitude",
      "Black Pendant",
      "Versago the Destroyer",
      "Dark King of the Abyss",
      "Stuffed Animal",
      "Horn Imp",
      "Clown Zombie",
      "Fiend Sword",
      "Mystic Clown",
    ],
  },
  {
    name: "Ghoul right of Rex",
    deck: {
      "The Judgement Hand": 3,
      Holograh: 3,
      "Oscillo Hero": 3,
      "Sonic Maid": 3,
      "Ancient Elf": 3,
      Mikazukinoyaiba: 1,
      "Fiend Reflection #1": 3,
      "Crass Clown": 3,
      "Maiden of the Moonlight": 3,
      "White Magical Hat": 3,
      "Mystical Sheep #1": 3,
      "Ill Witch": 1,
      "Talons of Shurilane": 1,
      "Red Medicine": 2,
      "Bright Castle": 1,
      Hinotama: 2,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Canada",
    payout: 0,
    ante: [
      "Mikazukinoyaiba",
      "Ill Witch",
      "White Magical Hat",
      "Bright Castle",
      "Holograh",
      "Mystical Sheep #1",
      "Talons of Shurilane",
      "Sonic Maid",
      "Oscillo Hero",
      "Crass Clown",
      "The Judgement Hand",
      "Fiend Reflection #1",
      "Ancient Elf",
      "Maiden of the Moonlight",
    ],
  },
  {
    name: "Mini Ghoul 1",
    deck: {
      Takuhee: 3,
      "Dark King of the Abyss": 3,
      "Nimble Momonga": 3,
      Trakadon: 3,
      "The Judgement Hand": 3,
      "Ice Water": 3,
      "Drill Bug": 3,
      "Giant Germ": 3,
      "Oscillo Hero": 3,
      Ansatsu: 2,
      "Sword Hunter": 1,
      "Aqua Snake": 3,
      "Red Medicine": 2,
      Hinotama: 2,
      "Beast Fangs": 1,
      "Bear Trap": 2,
    },
    lp: 2000,
    field: "Yami",
    location: "Duel Express",
    payout: 390,
    ante: [
      "Sword Hunter",
      "Ansatsu",
      "Giant Germ",
      "Nimble Momonga",
      "Aqua Snake",
      "Beast Fangs",
      "Drill Bug",
      "Ice Water",
      "Dark King of the Abyss",
      "Oscillo Hero",
      "Trakadon",
      "The Judgement Hand",
      "Takuhee",
    ],
  },
  {
    name: "Mini Ghoul 2",
    deck: {
      "Claw Reacher": 3,
      "One Who Hunts Souls": 3,
      Yaranzo: 3,
      "Skull Knight": 2,
      "Ogre of the Black Shadow": 3,
      "Witch of the Black Forest": 3,
      Solitude: 3,
      "Dark King of the Abyss": 3,
      "Warrior of Tradition": 2,
      "Gate Sword": 2,
      "Koumori Dragon": 3,
      "Witty Phantom": 3,
      Hinotama: 2,
      "Dark Energy": 1,
      "Red Medicine": 2,
      "Bear Trap": 2,
    },
    lp: 2000,
    field: "Yami",
    location: "Duel Express",
    payout: 310,
    ante: [
      "Skull Knight",
      "Warrior of Tradition",
      "Gate Sword",
      "Claw Reacher",
      "Solitude",
      "Dark Energy",
      "One Who Hunts Souls",
      "Dark King of the Abyss",
      "Witch of the Black Forest",
      "Witty Phantom",
      "Ogre of the Black Shadow",
      "Koumori Dragon",
      "Yaranzo",
    ],
  },
  {
    name: "Mini Ghoul 3",
    deck: {
      "Invader from Another Dimension": 3,
      "Talons of Shurilane": 2,
      "White Magical Hat": 3,
      "Fiend Reflection #1": 3,
      "Fiend Reflection #2": 3,
      "Oscillo Hero": 3,
      Seiyaryu: 2,
      "Ancient Elf": 3,
      "Musician King": 2,
      "Mystical Sheep #1": 3,
      "Sonic Maid": 3,
      "The Judgement Hand": 3,
      Hinotama: 2,
      "Red Medicine": 2,
      "Bright Castle": 1,
      "Bear Trap": 2,
    },
    lp: 2000,
    field: "Yami",
    location: "Duel Express",
    payout: 0,
    ante: [
      "Seiyaryu",
      "Musician King",
      "White Magical Hat",
      "Bright Castle",
      "Mystical Sheep #1",
      "Talons of Shurilane",
      "Sonic Maid",
      "Oscillo Hero",
      "The Judgement Hand",
      "Fiend Reflection #2",
      "Invader from Another Dimension",
      "Fiend Reflection #1",
      "Ancient Elf",
    ],
  },
  {
    name: "Mini Ghoul 4",
    deck: {
      "Flame Ghost": 3,
      "Turtle Bird": 2,
      Lisark: 3,
      "Flame Manipulator": 3,
      "Charubin the Fire Knight": 3,
      "Vermillion Sparrow": 2,
      Takuhee: 3,
      "Flying Kamakiri #1": 3,
      Kurama: 3,
      Fireyarou: 3,
      "Launcher Spider": 2,
      "Dragoness the Wicked Knight": 3,
      Hinotama: 2,
      Invigoration: 1,
      "Red Medicine": 2,
      "Bear Trap": 2,
    },
    lp: 2000,
    field: "Yami",
    location: "Egypt Exhibition",
    payout: 0,
    ante: [
      "Launcher Spider",
      "Kurama",
      "Vermillion Sparrow",
      "Turtle Bird",
      "Flame Manipulator",
      "Flame Ghost",
      "Invigoration",
      "Charubin the Fire Knight",
      "Dragoness the Wicked Knight",
      "Lisark",
      "Fireyarou",
      "Flying Kamakiri #1",
      "Takuhee",
    ],
  },
  {
    name: "Mini Ghoul 5",
    deck: {
      "Mushroom Man #2": 3,
      "Electric Lizard": 3,
      "Tripwire Beast": 3,
      "Bean Soldier": 3,
      "Electric Snake": 3,
      "Frenzied Panda": 3,
      "Mega Thunderball": 3,
      "Kaminari Attack": 2,
      "Wood Remains": 3,
      "Bolt Penguin": 3,
      "Leopard Girl": 2,
      "Twin-Headed Thunder Dragon": 2,
      Invigoration: 1,
      Hinotama: 2,
      "Red Medicine": 2,
      "Bear Trap": 2,
    },
    lp: 2000,
    field: "Yami",
    location: "Egypt Exhibition",
    payout: 0,
    ante: [
      "Mega Thunderball",
      "Twin-Headed Thunder Dragon",
      "Kaminari Attack",
      "Electric Lizard",
      "Electric Snake",
      "Leopard Girl",
      "Wood Remains",
      "Invigoration",
      "Bolt Penguin",
      "Frenzied Panda",
      "Mushroom Man #2",
      "Tripwire Beast",
      "Bean Soldier",
    ],
  },
  {
    name: "Ghoul (Domino City - right-under left)",
    deck: {
      "Dark King of the Abyss": 3,
      "Megirus Light": 3,
      "Big Eye": 3,
      "Wicked Mirror": 3,
      "Ryu-Kishin": 3,
      "Beast of Talwar": 1,
      "Needle Ball": 3,
      "The Shadow Who Controls the Dark": 3,
      "Hiro's Shadow Scout": 3,
      "Versago the Destroyer": 3,
      "Monster Eye": 2,
      "Candle of Fate": 2,
      Morinphen: 1,
      Hinotama: 2,
      "Red Medicine": 2,
      "Malevolent Nuzzler": 1,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Ghoul (Domino City - right-under right)",
    deck: {
      "Hitotsu-Me Giant": 3,
      "The Last Warrior from Another Planet": 1,
      Prisman: 3,
      Solitude: 3,
      "One Who Hunts Souls": 3,
      "M-Warrior #1": 3,
      "Dragoness the Wicked Knight": 3,
      Garoozis: 1,
      Doron: 2,
      "M-Warrior #2": 3,
      "Karate Man": 3,
      Ansatsu: 1,
      "Axe of Despair": 1,
      "Vishwar Randi": 3,
      "Battle Steer": 1,
      "Red Medicine": 2,
      Hinotama: 2,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Ghoul (Domino City - under Clock Tower Square)",
    deck: {
      Lisark: 2,
      "Winged Dragon, Guardian of the Fortress #2": 3,
      Wilmee: 2,
      Mavelus: 2,
      "Mystical Sheep #2": 2,
      "Silver Fang": 2,
      Wolf: 2,
      "Frenzied Panda": 2,
      Torike: 2,
      Niwatori: 3,
      "Nekogal #1": 3,
      Fusionist: 2,
      "Super War-Lion": 2,
      Peacock: 2,
      Tatsunootoshigo: 2,
      Hinotama: 2,
      "Red Medicine": 2,
      "Mystical Moon": 1,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Ghoul (Domino City - left-under)",
    deck: {
      "Kappa Avenger": 3,
      "Aqua Snake": 3,
      "Penguin Knight": 3,
      "Oscillo Hero #2": 3,
      Fireyarou: 3,
      "Fire Eye": 3,
      "Twin-Headed Thunder Dragon": 1,
      "Flame Champion": 1,
      "Charubin the Fire Knight": 3,
      "Electric Snake": 3,
      "Bolt Escargot": 1,
      "Electric Lizard": 3,
      Octoberser: 1,
      Hinotama: 2,
      "Gravedigger Ghoul": 2,
      Invigoration: 1,
      "Red Medicine": 2,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Ghoul (Domino City - right-up)",
    deck: {
      "Little D": 3,
      Mechaleon: 3,
      "Wicked Dragon with the Ersatz Head": 3,
      "Lesser Dragon": 3,
      "Two-Mouth Darkruler": 3,
      "Fairy Dragon": 3,
      "Yamatano Dragon Scroll": 3,
      "Serpent Marauder": 3,
      "Petit Dragon": 3,
      "B. Dragon Jungle King": 1,
      "Sword Arm of Dragon": 1,
      "Sky Dragon": 1,
      Mikazukinoyaiba: 1,
      "Red Medicine": 2,
      Hinotama: 2,
      "Gravedigger Ghoul": 2,
      "Dragon Treasure": 1,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Ghoul (Domino City - Clock Tower Square)",
    deck: {
      "Temple of Skulls": 3,
      Skelgon: 3,
      "Zombie Warrior": 3,
      "Dokuroizo the Grim Reaper": 3,
      "Dark Assailant": 3,
      "Wood Remains": 3,
      "Flame Ghost": 3,
      "Blue-Eyed Silver Zombie": 3,
      "Fire Reaper": 2,
      "Fiend's Hand": 2,
      "Three-Legged Zombies": 3,
      "Violet Crystal": 1,
      Hinotama: 2,
      "Gravedigger Ghoul": 2,
      "Red Medicine": 2,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Ghoul at Bonz",
    deck: {
      "Ice Water": 2,
      "Ill Witch": 1,
      Prisman: 2,
      "Succubus Knight": 1,
      "Amazon of the Seas": 2,
      "Mystic Tomato": 2,
      Arlownay: 2,
      "Nekogal #1": 2,
      "Mystical Sand": 1,
      "Leopard Girl": 1,
      "Bear Trap": 2,
      "Hungry Burger": 1,
      "Goddess of Whim": 2,
      "Water Omotics": 2,
      "Mushroom Man": 2,
      "Sonic Maid": 2,
      "Enchanting Mermaid": 2,
      "Bean Soldier": 2,
      "Rainbow Marine Mermaid": 1,
      "Rose Spectre of Dunn": 1,
      "Nekogal #2": 1,
      LaMoon: 1,
      "Black Pendant": 1,
      Hinotama: 2,
      "Red Medicine": 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Italy",
    payout: 280,
    ante: [
      "Ill Witch",
      "Succubus Knight",
      "LaMoon",
      "Rainbow Marine Mermaid",
      "Mushroom Man",
      "Leopard Girl",
      "Goddess of Whim",
      "Rose Spectre of Dunn",
      "Nekogal #2",
      "Arlownay",
      "Prisman",
      "Black Pendant",
      "Nekogal #1",
      "Ice Water",
      "Mystical Sand",
      "Enchanting Mermaid",
      "Sonic Maid",
      "Mystic Tomato",
      "Water Omotics",
      "Bean Soldier",
      "Amazon of the Seas",
      "Hungry Burger",
    ],
  },
  {
    name: "Ghoul (Domino Station)",
    deck: {
      Kumootoko: 3,
      Armaill: 3,
      "Hard Armor": 3,
      "Musician King": 3,
      "Ancient Tree of Enlightenment": 3,
      "Armored Starfish": 3,
      "Giant Rat": 3,
      "Boulder Tortoise": 1,
      Hyozanryu: 2,
      "Skull-Mark Lady Bug": 3,
      "30,000-Year White Turtle": 1,
      "Total Defense Shogun": 1,
      "Darkfire Dragon": 3,
      "Queen Bird": 1,
      Hinotama: 2,
      "Red Medicine": 2,
      "Steel Shell": 1,
      Eatgaboon: 2,
    },
    lp: 8000,
    field: "Yami",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Millennium Guardian 1",
    deck: {
      "Mystical Sheep #2": 3,
      LaMoon: 3,
      "Goddess of Whim": 3,
      "Hourglass of Life": 3,
      "The Bewitching Phantom Thief": 3,
      Genin: 3,
      "Happy Lover": 3,
      "Necrolancer the Timelord": 3,
      "Muse-A": 3,
      "Wicked Mirror": 3,
      Mikazukinoyaiba: 1,
      "Bright Castle": 1,
      "Red Medicine": 2,
      "Lucky Trinket": 3,
      "Black Pendant": 1,
      Eatgaboon: 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Egypt Exhibition",
    payout: 400,
    ante: [
      "LaMoon",
      "Necrolancer the Timelord",
      "The Bewitching Phantom Thief",
      "Lucky Trinket",
      "Genin",
      "Happy Lover",
      "Hourglass of Life",
      "Wicked Mirror",
      "Mystical Sheep #2",
      "Black Pendant",
      "Red Medicine",
      "Goddess of Whim",
      "Muse-A",
      "Bright Castle",
      "Eatgaboon",
      "Mikazukinoyaiba",
    ],
  },
  {
    name: "Millennium Guardian 2",
    deck: {
      Morinphen: 1,
      "Hiro's Shadow Scout": 3,
      Genin: 3,
      "Candle of Fate": 3,
      "Djinn the Watcher of the Wind": 3,
      "Boo Koo": 3,
      "Flame Manipulator": 3,
      "Claw Reacher": 3,
      "White Magical Hat": 3,
      "Giant Germ": 3,
      "Needle Ball": 3,
      "Necrolancer the Timelord": 3,
      "Spirit of the Mountain": 1,
      "Beast of Talwar": 1,
      "Exile of the Wicked": 1,
      "Restructer Revolution": 1,
      "Last Day of Witch": 1,
      Eatgaboon: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Domino Pier",
    payout: 400,
    ante: [
      "White Magical Hat",
      "Flame Manipulator",
      "Necrolancer the Timelord",
      "Djinn the Watcher of the Wind",
      "Claw Reacher",
      "Genin",
      "Giant Germ",
      "Candle of Fate",
      "Boo Koo",
      "Needle Ball",
      "Morinphen",
      "Spirit of the Mountain",
      "Hiro's Shadow Scout",
      "Eatgaboon",
      "Exile of the Wicked",
      "Last Day of Witch",
      "Restructer Revolution",
      "Beast of Talwar",
    ],
  },
  {
    name: "Millennium Guardian 3",
    deck: {
      "Wicked Dragon with the Ersatz Head": 3,
      "Goddess of Whim": 3,
      "Toad Master": 3,
      "Muse-A": 3,
      "Necrolancer the Timelord": 3,
      "Electric Snake": 3,
      "Thunder Dragon": 1,
      "Nightmare Scorpion": 3,
      "Princess of Tsurugi": 3,
      "Electric Lizard": 3,
      Dissolverock: 2,
      "Two-Mouth Darkruler": 3,
      "Stone D.": 1,
      "Succubus Knight": 1,
      "Red Medicine": 2,
      Eatgaboon: 3,
    },
    lp: 8000,
    field: "Arena",
    location: "Italy",
    payout: 400,
    ante: [
      "Nightmare Scorpion",
      "Toad Master",
      "Necrolancer the Timelord",
      "Two-Mouth Darkruler",
      "Dissolverock",
      "Princess of Tsurugi",
      "Wicked Dragon with the Ersatz Head",
      "Red Medicine",
      "Thunder Dragon",
      "Stone D.",
      "Goddess of Whim",
      "Electric Snake",
      "Muse-A",
      "Electric Lizard",
      "Succubus Knight",
      "Eatgaboon",
    ],
  },
  {
    name: "Millennium Guardian 4",
    deck: {
      "Karate Man": 3,
      "Flame Manipulator": 3,
      "Electric Snake": 3,
      Kurama: 3,
      "Frenzied Panda": 3,
      "Nimble Momonga": 3,
      "Steel Fan Fighter": 1,
      "Metal Dragon": 1,
      Alinsection: 3,
      Holograh: 3,
      "Mystical Sheep #1": 3,
      Solitude: 3,
      Minar: 3,
      "Thunder Dragon": 1,
      "Red Medicine": 2,
      "Bear Trap": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "China",
    payout: 1000,
    ante: [
      "Flame Manipulator",
      "Karate Man",
      "Nimble Momonga",
      "Frenzied Panda",
      "Solitude",
      "Holograh",
      "Kurama",
      "Red Medicine",
      "Metal Dragon",
      "Thunder Dragon",
      "Electric Snake",
      "Alinsection",
      "Minar",
      "Mystical Sheep #1",
      "Bear Trap",
      "Steel Fan Fighter",
    ],
  },
  {
    name: "Millennium Guardian 5",
    deck: {
      "Mystical Sheep #1": 3,
      "Nekogal #1": 2,
      "Wicked Dragon with the Ersatz Head": 3,
      "Djinn the Watcher of the Wind": 3,
      "Twin-Headed Thunder Dragon": 1,
      "Electric Snake": 3,
      "Sword Hunter": 1,
      Ansatsu: 1,
      Wetha: 3,
      "Aqua Snake": 3,
      "Muse-A": 3,
      "Zombie Warrior": 3,
      "The Statue of Easter Island": 1,
      "Crawling Dragon": 1,
      Morinphen: 1,
      Akihiron: 1,
      "Malevolent Nuzzler": 2,
      Sparks: 2,
      "Black Pendant": 2,
      "Heavy Storm": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Canada",
    payout: 1000,
    ante: [
      "Horn Imp",
      "Humanoid Slime",
      "Yamatano Dragon Scroll",
      "Banisher of the Light",
      "Big Eye",
      "Heavy Storm",
      "Petit Dragon",
      "Mother Grizzly",
      "One-Eyed Shield Dragon",
      "Wicked Dragon with the Ersatz Head",
      "Earthbound Spirit",
      "Lesser Dragon",
      "Fairy Dragon",
      "Shadow Spell",
    ],
  },
  {
    name: "Paradox Guardian 1",
    deck: {
      "Darkfire Dragon": 3,
      "The Stern Mystic": 3,
      Takriminos: 3,
      "Maiden of the Moonlight": 3,
      Kuriboh: 3,
      "Tri-Horned Dragon": 1,
      "Faith Bird": 3,
      Seiyaryu: 1,
      "Crab Turtle": 1,
      "Zera The Mant": 1,
      "The Immortal of Thunder": 3,
      "Rare Fish": 3,
      "Skull Knight": 1,
      Sengenjin: 1,
      "Cosmo Queen": 1,
      "Restructer Revolution": 3,
      Multiply: 3,
      Yami: 3,
    },
    lp: 3000,
    field: "Arena",
    location: "China",
    payout: 0,
    ante: [
      "The Stern Mystic",
      "Kuriboh",
      "Faith Bird",
      "Darkfire Dragon",
      "Rare Fish",
      "Yami",
      "Takriminos",
      "Seiyaryu",
      "Zera The Mant",
      "The Immortal of Thunder",
      "Maiden of the Moonlight",
      "Sengenjin",
      "Tri-Horned Dragon",
      "Skull Knight",
      "Cosmo Queen",
      "Crab Turtle",
      "Multiply",
    ],
  },
  {
    name: "Paradox Guardian 2",
    deck: {
      "Darkfire Dragon": 3,
      "Dark Magician": 3,
      "Cosmo Queen": 1,
      "The Immortal of Thunder": 3,
      "Maiden of the Moonlight": 3,
      Takriminos: 3,
      "Time Wizard": 3,
      "Baby Dragon": 3,
      "The Stern Mystic": 3,
      "Rare Fish": 3,
      "Faith Bird": 3,
      "Tri-Horned Dragon": 1,
      Sengenjin: 1,
      "Zera The Mant": 1,
      "Restructer Revolution": 3,
      Yami: 3,
    },
    lp: 3000,
    field: "Arena",
    location: "China",
    payout: 0,
    ante: [
      "Baby Dragon",
      "The Stern Mystic",
      "Time Wizard",
      "Dark Magician",
      "Faith Bird",
      "Darkfire Dragon",
      "Rare Fish",
      "Yami",
      "Takriminos",
      "Zera The Mant",
      "The Immortal of Thunder",
      "Maiden of the Moonlight",
      "Sengenjin",
      "Tri-Horned Dragon",
      "Cosmo Queen",
    ],
  },
  {
    name: "Paradox Guardian 3",
    deck: {
      "Maiden of the Moonlight": 3,
      "The Stern Mystic": 3,
      "Faith Bird": 3,
      Takriminos: 3,
      "The Wicked Worm Beast": 3,
      "Rare Fish": 3,
      "The Immortal of Thunder": 3,
      "Cosmo Queen": 1,
      Trakadon: 3,
      "Tri-Horned Dragon": 1,
      "Darkfire Dragon": 3,
      "The Judgement Hand": 3,
      Seiyaryu: 1,
      "Zera The Mant": 1,
      "Skull Knight": 1,
      "Crab Turtle": 1,
      Sengenjin: 1,
      "Restructer Revolution": 3,
    },
    lp: 3000,
    field: "Arena",
    location: "China",
    payout: 0,
    ante: [
      "The Stern Mystic",
      "The Wicked Worm Beast",
      "Faith Bird",
      "The Judgement Hand",
      "Darkfire Dragon",
      "Rare Fish",
      "Takriminos",
      "Seiyaryu",
      "Zera The Mant",
      "The Immortal of Thunder",
      "Trakadon",
      "Maiden of the Moonlight",
      "Sengenjin",
      "Tri-Horned Dragon",
      "Skull Knight",
      "Cosmo Queen",
      "Crab Turtle",
    ],
  },
  {
    name: "Paradox Guardian 4",
    deck: {
      "Maiden of the Moonlight": 3,
      "The Immortal of Thunder": 3,
      "Darkfire Dragon": 3,
      "Celtic Guardian": 3,
      "Faith Bird": 3,
      Sengenjin: 1,
      "Zera The Mant": 1,
      Takriminos: 3,
      "The Stern Mystic": 3,
      "Rare Fish": 3,
      Seiyaryu: 1,
      "Tri-Horned Dragon": 1,
      "Skull Knight": 1,
      "Crab Turtle": 1,
      "Cosmo Queen": 1,
      "Restructer Revolution": 3,
      Sogen: 3,
      "Legendary Sword": 3,
    },
    lp: 3000,
    field: "Arena",
    location: "China",
    payout: 0,
    ante: [
      "The Stern Mystic",
      "Celtic Guardian",
      "Faith Bird",
      "Darkfire Dragon",
      "Rare Fish",
      "Legendary Sword",
      "Sogen",
      "Takriminos",
      "Seiyaryu",
      "Zera The Mant",
      "The Immortal of Thunder",
      "Maiden of the Moonlight",
      "Sengenjin",
      "Tri-Horned Dragon",
      "Skull Knight",
      "Cosmo Queen",
      "Crab Turtle",
    ],
  },
  {
    name: "Paradox Guardian 5",
    deck: {
      "Curse of Dragon": 3,
      "Winged Dragon, Guardian of the Fortress #1": 3,
      Takriminos: 3,
      "Rare Fish": 3,
      "Maiden of the Moonlight": 3,
      "The Immortal of Thunder": 3,
      "Darkfire Dragon": 3,
      "The Stern Mystic": 3,
      "Blackland Fire Dragon": 3,
      "Faith Bird": 3,
      "Gaia The Fierce Knight": 3,
      "Gaia the Dragon Champion": 1,
      Mountain: 3,
      "Restructer Revolution": 3,
    },
    lp: 3000,
    field: "Arena",
    location: "China",
    payout: 0,
    ante: [
      "Winged Dragon, Guardian of the Fortress #1",
      "Blackland Fire Dragon",
      "The Stern Mystic",
      "Gaia the Dragon Champion",
      "Gaia The Fierce Knight",
      "Curse of Dragon",
      "Faith Bird",
      "Darkfire Dragon",
      "Rare Fish",
      "Mountain",
      "Takriminos",
      "The Immortal of Thunder",
      "Maiden of the Moonlight",
    ],
  },
  {
    name: "Chevaliers (Cable Car)",
    deck: {
      "Maha Vailo": 2,
      "Mushroom Man #2": 2,
      "Talons of Shurilane": 1,
      "Minomushi Warrior": 2,
      "Maiden of the Moonlight": 2,
      "Darkfire Dragon": 2,
      "Mother Grizzly": 2,
      Takuhee: 2,
      "Magical Ghost": 2,
      "Rainbow Marine Mermaid": 1,
      "Dark King of the Abyss": 2,
      "Tripwire Beast": 2,
      Togex: 1,
      "Metal Dragon": 1,
      "Crow Goblin": 1,
      "Dice Armadillo": 1,
      "Musician King": 1,
      Spikebot: 1,
      Skelgon: 1,
      "Swords of Revealing Light": 1,
      Ookazi: 1,
      Sogen: 1,
      Umi: 1,
      Wasteland: 1,
      "Goblin's Secret Remedy": 1,
      Forest: 1,
      Mountain: 1,
      Yami: 1,
      "Invisible Wire": 2,
    },
    lp: 10000,
    field: "Arena",
    location: "Pegasus Island",
    payout: 0,
    ante: [
      "Goblin's Secret Remedy",
      "Rainbow Marine Mermaid",
      "Musician King",
      "Dice Armadillo",
      "Togex",
      "Spikebot",
      "Ookazi",
      "Metal Dragon",
      "Crow Goblin",
      "Invisible Wire",
      "Skelgon",
      "Talons of Shurilane",
      "Dark King of the Abyss",
      "Mushroom Man #2",
      "Minomushi Warrior",
      "Tripwire Beast",
      "Mother Grizzly",
      "Magical Ghost",
      "Takuhee",
      "The Immortal of Thunder",
      "Maiden of the Moonlight",
      "Maha Vailo",
    ],
  },
  {
    name: "Chevalier 1 (West)",
    deck: {
      "Minomushi Warrior": 3,
      "Karbonala Warrior": 3,
      "Mystic Horseman": 3,
      "Tripwire Beast": 3,
      "Breath of Light": 1,
      "Kaminari Attack": 3,
      "Steel Ogre Grotto #2": 2,
      "Giant Flea": 3,
      "The Immortal of Thunder": 3,
      "Nekogal #2": 3,
      "Twin-Headed Thunder Dragon": 1,
      "Empress Judge": 1,
      "Flame Cerebrus": 1,
      "Warrior Elimination": 1,
      Ookazi: 1,
      "Eternal Rest": 1,
      "Exile of the Wicked": 1,
      "Last Day of Witch": 1,
      "Eradicating Aerosol": 1,
      "Swords of Revealing Light": 1,
      "Goblin's Secret Remedy": 1,
      "Eternal Drought": 1,
      "Stain Storm": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Chevalier 1 (East)",
    deck: {
      "Breath of Light": 1,
      "Darkfire Dragon": 3,
      "Flying Kamakiri #2": 3,
      Grappler: 3,
      Lisark: 3,
      "Armored Lizard": 3,
      Fireyarou: 3,
      "Flame Champion": 3,
      "Deepsea Shark": 3,
      Skullbird: 3,
      "Roaring Ocean Snake": 1,
      "Punished Eagle": 1,
      Ookazi: 1,
      "Goblin's Secret Remedy": 1,
      "Swords of Revealing Light": 1,
      "Warrior Elimination": 1,
      "Eternal Rest": 1,
      "Exile of the Wicked": 1,
      "Last Day of Witch": 1,
      "Eradicating Aerosol": 1,
      "Eternal Drought": 1,
      "Stain Storm": 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Chevalier 2 (West)",
    deck: {
      "Maiden of the Moonlight": 3,
      "Oscillo Hero": 3,
      "Fiend Reflection #1": 3,
      "Talons of Shurilane": 1,
      "Sea King Dragon": 2,
      "Man-Eating Treasure Chest": 3,
      "High Tide Gyojin": 3,
      "Mystical Sheep #1": 3,
      "Water Omotics": 3,
      "Violent Rain": 3,
      "Enchanting Mermaid": 3,
      Grappler: 3,
      Ookazi: 2,
      "Goblin's Secret Remedy": 2,
      "Swords of Revealing Light": 1,
      "Invisible Wire": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Chevalier 2 (East)",
    deck: {
      "Karate Man": 3,
      "Kanan the Swordmistress": 2,
      Holograh: 3,
      Mikazukinoyaiba: 1,
      "Sword Hunter": 1,
      "Maha Vailo": 3,
      "Destroyer Golem": 3,
      "The Statue of Easter Island": 3,
      "Shining Friendship": 3,
      "Ancient Elf": 3,
      "White Magical Hat": 3,
      Torike: 3,
      "Millennium Golem": 1,
      "Dark Witch": 1,
      "Swords of Revealing Light": 1,
      Ookazi: 2,
      "Goblin's Secret Remedy": 2,
      "Invisible Wire": 2,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
  {
    name: "Chevalier 3 (West)",
    deck: {
      "Darkfire Dragon": 3,
      Fireyarou: 3,
      "Magical Ghost": 3,
      "The Last Warrior from Another Planet": 1,
      "Metal Dragon": 1,
      "Fire Kraken": 3,
      "Whiptail Crow": 3,
      Lisark: 3,
      Takuhee: 3,
      "Skull Red Bird": 3,
      "Stuffed Animal": 2,
      "Giant Mech-Soldier": 1,
      "Cannon Soldier": 3,
      "Turtle Bird": 1,
      Ookazi: 2,
      "Goblin's Secret Remedy": 2,
      "Swords of Revealing Light": 1,
      "Invisible Wire": 2,
    },
    lp: 10000,
    field: "Mountain",
    location: "Dungeon",
    payout: 0,
    ante: [
      "The Last Warrior from Another Planet",
      "Goblin's Secret Remedy",
      "Ookazi",
      "Metal Dragon",
      "Invisible Wire",
      "Giant Mech-Soldier",
      "Turtle Bird",
      "Stuffed Animal",
      "Lisark",
      "Fireyarou",
      "Magical Ghost",
      "Cannon Soldier",
      "Takuhee",
      "Skull Red Bird",
      "Fire Kraken",
      "Whiptail Crow",
    ],
  },
  {
    name: "Chevalier 3 (East)",
    deck: {
      "Dark King of the Abyss": 3,
      "Giant Germ": 2,
      "Bolt Penguin": 3,
      "Ushi Oni": 1,
      "Twin-Headed Thunder Dragon": 1,
      "The Immortal of Thunder": 3,
      "Baron of the Fiend Sword": 3,
      "Versago the Destroyer": 3,
      "Tripwire Beast": 3,
      "Mystic Tomato": 3,
      "Nimble Momonga": 3,
      "Kuwagata A": 3,
      "Leopard Girl": 1,
      "Warrior of Tradition": 1,
      Ookazi: 2,
      "Goblin's Secret Remedy": 2,
      "Swords of Revealing Light": 1,
      "Invisible Wire": 2,
    },
    lp: 10000,
    field: "Mountain",
    location: "Dungeon",
    payout: 0,
    ante: [
      "Goblin's Secret Remedy",
      "Ookazi",
      "Twin-Headed Thunder Dragon",
      "Warrior of Tradition",
      "Leopard Girl",
      "Giant Germ",
      "Nimble Momonga",
      "Versago the Destroyer",
      "Bolt Penguin",
      "Dark King of the Abyss",
      "Kuwagata A",
      "Ushi Oni",
      "Tripwire Beast",
      "Mystic Tomato",
      "The Immortal of Thunder",
      "Baron of the Fiend Sword",
    ],
  },
  {
    name: "Chevalier 4",
    deck: {
      "Ground Attacker Bugroth": 2,
      "Man-Eating Treasure Chest": 2,
      "Skull Knight": 1,
      "Talons of Shurilane": 1,
      "The Immortal of Thunder": 2,
      "Rare Fish": 2,
      "Fiend Sword": 2,
      "Roaring Ocean Snake": 1,
      "Firewing Pegasus": 1,
      "Flame Cerebrus": 1,
      "Mystic Clown": 2,
      "Darkfire Soldier #2": 2,
      "Earthbound Spirit": 1,
      "Mushroom Man #2": 2,
      "Skull Red Bird": 2,
      "Guardian of the Throne Room": 2,
      Mikazukinoyaiba: 1,
      "Twin-Headed Thunder Dragon": 1,
      "Sky Dragon": 1,
      "Swords of Revealing Light": 1,
      "Goblin's Secret Remedy": 2,
      Ookazi: 2,
      Forest: 1,
      Yami: 1,
      Wasteland: 1,
      Mountain: 1,
      Umi: 1,
      Sogen: 1,
    },
    lp: 8000,
    field: "Arena",
    location: "Clock Tower Square",
    payout: 0,
    ante: [],
  },
];

const routeDuellists = routeDuellistNames.reduce(
  (arr: Duellist[], duellistName: string) => [
    ...arr,
    duellists.find((d) => d.name === duellistName)!,
  ],
  []
);

export default routeDuellists;
